...
- **单会话层**：字节稳定前缀（24 条工程纪律，继承自 Semantix）——命中率 90%+（长会话稳态）
...
| 模式 | 适用 | 工作量 |
|---|---|---|
| A. agent skill 自助接入 | 任何框架（Semantix/Claude Code/自定义） | 安装脚本 + 配置（≤1 小时） |
| B. LangChain 中间件 | 已有 LangChain 应用 | 两个挂点（改写+提取），示例代码现成 |
| C. harness fork | 深度集成（Semantix 用户） | 事件级旁路 + 注入 hook（已实现） |

## 收益口径（对外只能引这些）

| 数字 | 口径 | 出处 |
|---|---|---|
| **90%+** 单会话缓存命中 | 长会话稳态（官方口径） | semantix-kvcache-mechanisms.md |
| **71%** 单会话缓存命中 | 独立任务冷会话实测（49 次） | 同上 |
| **79.8%** 成本节省 | 跨会话复用演示实测（合成数据） | m0-cost-comparison.md |
| **≥70%** 跨会话命中目标 | 真实数据验证门槛（未达先调提取粒度） | m0-gate.md |
| **24h TTL** 前缀缓存 | DeepSeek 厂商缓存策略（记忆本体持久存储，无 TTL） | semantix-kvcache-mechanisms.md:39 |
...
- `docs/reports/semantix-kvcache-mechanisms.md`（单会话机制，24 条纪律）
