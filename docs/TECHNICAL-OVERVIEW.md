# Semantix — Technical Overview

> This is the full technical companion to the [README](../README.md). The front page stays short on purpose; everything below is for readers who want the architecture, the mechanisms, the current status, and the evidence behind the numbers.
>
> 中文版本见 [TECHNICAL-OVERVIEW.zh-CN.md](./TECHNICAL-OVERVIEW.zh-CN.md)。

**Contents**: [Why Semantix](#why-semantix) · [Reuse Visualization](#reuse-visualization) · [Semantic Slice Library](#semantic-slice-library) · [Three-Layer Semantic Cache](#three-layer-semantic-cache) · [Adaptive Kernel Scheduler](#adaptive-kernel-scheduler) · [Speculative Prefetch](#speculative-prefetch) · [Self-Evolving Optimization](#self-evolving-optimization) · [Architecture](#architecture) · [Repository Module Guide](#repository-module-guide) · [How a Request Flows](#how-a-request-flows) · [Harness + Kernel](#harness--kernel) · [Design Principles](#design-principles) · [Project Status](#project-status) · [Roadmap](#roadmap) · [Target Metrics](#target-metrics) · [Evaluation Philosophy](#evaluation-philosophy) · [What Semantix Is Not](#what-semantix-is-not) · [Acknowledgements](#acknowledgements)
...
> **Cross-session reuse you can see.** Semantix renders the three reuse signals — hit rate, cost saved, source session — as one-glance CLI output (the Agile 1 DoD "reuse visualization", shipped as U28–U31). Every block below is **real output** captured from a small demo library (4 extracted sessions); the real-data measurement lives in [`reports/m0-cost-comparison.md`](./reports/m0-cost-comparison.md) (**79.8%** cost saved on synthetic replay).
...
> **Status: partially shipped** — L2 deterministic injection (`semantix inject`) and L3 verified reuse (`kernel/fingerprint` + `kernel/judge` + `kernel/promote`, accepted per [`reports/issue-08-acceptance.md`](./reports/issue-08-acceptance.md)) are implemented; real-harness end-to-end validation is pending.
...
> **Status: shipped (MVP)** — `kernel/sched.RuleDecider` implements parallel groups, a behavior-learning gate, model tiering, and prefetch hints (see [`Agile路线图.md`](./Agile路线图.md) H3); provider-level model switching is planned.
...
> **Status: shipped (MVP)** — `kernel/prefetch` ships `Planner` (offline, Issue 62) + `MatrixPrefetcher` (online, hit/waste feedback) plus a `Runner`; wired into the scheduler via `prefetch.AsPlanFunc` (see [`Agile路线图.md`](./Agile路线图.md) H5).
...
│                     Agent Harness                        │
│                                                         │
│  semantix-agent · Claude Code-style agents · custom     │
└──────────────────────────┬──────────────────────────────┘
                           │
                    events │ decisions
                           │
...
| Specs and evidence | [`docs`](.) | Architecture, security, roadmap, specifications and acceptance reports that separate design targets from measured results. | Review the linked acceptance report for each shipped unit. |
...
Semantix ships in two halves that solve different layers of the agent-infrastructure problem: **`semantix-agent`**, the bundled agent harness (derived from the DeepSeek-Reasonix architecture baseline), and **`semantix`**, the cross-session optimization kernel. Since v0.3.0 the release bundle ships both binaries together with a shared install script and example config.

| Capability                      | semantix-agent (harness)  | semantix (kernel)          |
| ------------------------------- | ------------------------- | -------------------------- |
...
The kernel is not designed to replace the harness.

It is designed to make harnesses like `semantix-agent` — and any other agent runtime it attaches to — more efficient over time.
...
> **Semantix v0.7.1 is released** (2026-08-22, brand unification — the repo, binaries and on-disk contracts now ship under the Semantix name; see [`releases/v0.7.1.md`](./releases/v0.7.1.md)), bundling `semantix-agent` + `semantix` + `semantix-gateway`. **Agile 2 (self-evolving loop) closed with v0.6.0** (2026-08-21): kernel-orchestrated scheduling, speculative prefetch, and the closed evolution loop (U37–U43) shipped, plus the first batch of GLM-5.x cache adaptation (prefix hygiene + per-provider hit metering, GLM P0). The remaining gate before scaling up is **real-data validation of the cross-session hit rate** ([#58](https://github.com/Gnosil/semantix/issues/58), Agile 1's v1.0 gate).
...
**First integration target shipped**: since v0.3.0 the release bundle packages **semantix-agent + semantix** together (per-platform archives, install script, example config).

**Gate**: M0-Gate passed conditionally (2026-08-09) — technical feasibility and cost savings (79.8% on synthetic replay, [`reports/m0-cost-comparison.md`](./reports/m0-cost-comparison.md)) are verified; **real-data cross-session hit rate ≥ 70%** (`semantix verify`) is the remaining gate, per [`reports/m0-gate.md`](./reports/m0-gate.md).

**Agile roadmap**: Agile 1 (first downloadable, brandable agent) is in progress — M0 shipped, M1 nearly complete, with #58 (real-data hit rate) as the remaining gate. Agile 2 (self-evolving loop) shipped and released as **v0.6.0** (2026-08-21); Agile 3 (multi-harness ecosystem) is defined in [`Agile路线图.md`](./Agile路线图.md).

---

## Roadmap

Execution is organized in **Agile cycles** — one downloadable milestone per Agile (see [`Agile路线图.md`](./Agile路线图.md)). The technical phases P0–P5 map onto them as follows:
...
| Cost per task                        |   ≥ 50% reduction vs. baseline | 79.8% on synthetic replay ([`reports/m0-cost-comparison.md`](./reports/m0-cost-comparison.md)) |
| Prefetch utilization                 | ≥ 30% of eligible wait windows | pending — P4 shipped as MVP, utilization not yet measured           |
...
Semantix uses **DeepSeek-Reasonix** as its initial architecture baseline; the bundled `semantix-agent` harness derives from that lineage.

DeepSeek-Reasonix's work on:
...
* [`QUICKSTART.md`](./QUICKSTART.md) — install, 30-second demo, command reference, security conventions.
* [`Agent-Infra-架构设计.md`](./Agent-Infra-架构设计.md) — full architecture design (problem definition, SSL, L1/L2/L3, scheduling, prefetch, evolution, risks, roadmap, metrics).
* [`总体架构-流程树.md`](./总体架构-流程树.md) — structured end-to-end request lifecycle.
* [`Agile路线图.md`](./Agile路线图.md) — Agile 1–3 roadmap and DoD.
* [`Security-安全设计.md`](./Security-安全设计.md) and [`../SECURITY.md`](../SECURITY.md) — threat model and security mechanisms.
* [`reports/glm-spike-week.md`](./reports/glm-spike-week.md) — controlled GLM cache observations, including an AtomClub→GLM-5.2 gateway pilot (96–98% cached input at 0–120 s, 28% at ~5 min, Anthropic Messages usage-field mapping, strict-prefix invalidation). Gateway-specific; no direct Z.AI claims.
* [`../agent-skill/SKILL.md`](../agent-skill/SKILL.md) — self-serve integration for any harness.
* [`../site`](../site) — marketing site and product docs, including the blog on semantic caching and cross-session reuse.
