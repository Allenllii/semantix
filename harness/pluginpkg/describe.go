...
func InstalledNames(semantixHome string) ([]string, error) {
...
func InstalledListText(semantixHome string) (string, error) {
	st, err := LoadState(semantixHome)
	if err != nil {
		return "", err
	}
	if len(st.Plugins) == 0 {
		return "plugins: none installed\ninstall: semantix-agent plugin install <source> --yes, or use Settings -> Plugins", nil
	}
	...
	for _, p := range st.Plugins {
		...
		counts := pluginCapabilityText(semantixHome, p)
		if summary != "" && counts != "" {
...
func InstalledShowText(semantixHome, name string) (string, error) {
	p, ok, err := FindInstalled(semantixHome, name)
	if err != nil {
		return "", err
	}
	if !ok {
	...
	root := ResolveRoot(semantixHome, p.Root)
...
func FindInstalled(semantixHome, name string) (InstalledPlugin, bool, error) {
	st, err := LoadState(semantixHome)
	if err != nil {
...
func pluginCapabilityText(semantixHome string, p InstalledPlugin) string {
	root := ResolveRoot(semantixHome, p.Root)
...
func RuntimeTrustText(rt *RuntimeSpec) string {
	...
	b.WriteString("  risk: the runtime process runs inside Semantix — it can read the full session and environment, bypass permissions, and operate this machine directly.\n")
	return b.String()
}
