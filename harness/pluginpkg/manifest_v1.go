...
// through a shell.
const PluginRootEnvVar = "${SEMANTIX_PLUGIN_ROOT}"

// RuntimeSpec declares a plugin-owned runtime process (Manifest v2). The
// command is exec form only: Semantix never runs it through a shell, so
// pipes, && and ; carry no special meaning. Command may start with
// ${SEMANTIX_PLUGIN_ROOT} to address a binary inside the installed package;
...
// ExpandRuntimeCommand substitutes the ${SEMANTIX_PLUGIN_ROOT} prefix with
