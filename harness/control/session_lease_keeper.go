...
// that binds session files for writing (the CLI chat/run commands, `semantix
...
const SessionLeaseCloseHint = "close the other Semantix window or process first"
...
func SessionInUseMessage(err error) string {
	const fallback = "this session is in use by another Semantix window or process"
	...
	fmt.Fprintf(&b, "this session is in use by another Semantix process (pid %d", info.PID)
