...
func TestClassifyRef(t *testing.T) {
	...
	files := map[string]bool{
		"src/main.go": true,
		"README.md":   true,
		".semantix/attachments/clipboard-20260601-010203.000000.png": true,
		".semantix/attachments/clipboard-20260601-010203.000000.yml": true,
		".semantix/attachments/clipboard-20260601-010203.000000.zip": true,
	}
	...
	cases := []struct {
		...
		{".semantix/attachments/clipboard-20260601-010203.000000.png", true, refImage},
		{".semantix/attachments/clipboard-20260601-010203.000000.yml", true, refFile},
		{".semantix/attachments/clipboard-20260601-010203.000000.zip", true, refFile},
...
func TestResolveRefsAttachmentKinds(t *testing.T) {
	temp := t.TempDir()
	attachmentsDir := filepath.Join(temp, ".semantix", "attachments")
	if err := os.MkdirAll(attachmentsDir, 0o755); err != nil {
		t.Fatal(err)
	}
	ymlRef := filepath.ToSlash(".semantix/attachments/config.yml")
	zipRef := filepath.ToSlash(".semantix/attachments/archive.zip")
	pngRef := filepath.ToSlash(".semantix/attachments/shot.png")
	if err := os.WriteFile(filepath.Join(temp, filepath.FromSlash(ymlRef)), []byte("name: semantix\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(block, `<file path="`+ymlRef+`">`) || !strings.Contains(block, "name: semantix") {
...
func TestWorkspaceImageRefsAlsoAttachAsModelImages(t *testing.T) {
	...
	attachment := filepath.Join(workspace, ".semantix", "attachments", "shot.png")
	...
	if refs[1].kind != refImage || refs[1].path != ".semantix/attachments/shot.png" {
