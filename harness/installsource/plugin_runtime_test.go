...
func writeRuntimePlugin(t *testing.T, root string) {
	t.Helper()
	writeFile(t, filepath.Join(root, "semantix-plugin.json"), `{
  ...
  "runtime": {
    "command": "${SEMANTIX_PLUGIN_ROOT}/bin/rtsync",
    "args": ["--serve"],
    "required": true,
	...
	writeFile(t, filepath.Join(root, "themes", "neon.semantix-theme"), "theme bytes")
...
func TestPluginRuntimePlanCarriesFullTrust(t *testing.T) {
	...
	for _, reason := range act.RiskReasons {
		if strings.HasPrefix(reason, "FULL TRUST:") {
			fullTrust = true
			if !strings.Contains(reason, "${SEMANTIX_PLUGIN_ROOT}/bin/rtsync --serve") {
	...
	if rt.Command != "${SEMANTIX_PLUGIN_ROOT}/bin/rtsync" || !rt.FullTrust {
	...
	if rtJSON["fullTrust"] != true || rtJSON["command"] != "${SEMANTIX_PLUGIN_ROOT}/bin/rtsync" {
...
func TestPluginLegacyPlanOmitsRuntimeFields(t *testing.T) {
	...
	writeFile(t, filepath.Join(src, "semantix-plugin.json"), `{"apiVersion":"semantix.io/plugin/v2","name":"legacy","skills":["skills"]}`)
