...
// known_hosts files (read-only) and a Semantix-managed file (read-write, TOFU).
type HostKeyPolicy struct {
	...
	// ManagedPath is the Semantix-managed known_hosts file that accepted TOFU
...
func (hostKeyLookupProbe) Type() string    { return "semantix-host-key-lookup-probe" }
func (hostKeyLookupProbe) Marshal() []byte { return []byte("semantix-host-key-lookup-probe") }
