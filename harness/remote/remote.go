// Package remote is the SSH transport for Semantix's remote module: host
// resolution ([remote] config + ~/.ssh/config), authentication, host-key
// verification (system known_hosts read-only + a Semantix-managed TOFU file),
...
// through this package — remote workspaces are driven by a `semantix-agent serve`
