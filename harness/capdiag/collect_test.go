...
func TestCollectStaticNoNetworkSideEffects(t *testing.T) {
	...
	write(t, filepath.Join(root, ".semantix", "skills", "demo", "SKILL.md"),
		"---\nname: demo\ndescription: project demo\n---\nbody\n")
	write(t, filepath.Join(home, ".semantix", "skills", "demo", "SKILL.md"),
		"---\nname: demo\ndescription: global demo\n---\nbody\n")
	write(t, filepath.Join(root, ".semantix", "skills", "nodesc", "SKILL.md"),
		"---\nname: nodesc\n---\nbody\n")
	write(t, filepath.Join(root, ".semantix", "commands", "hi.md"),
		"---\ndescription: project hi\n---\nP $ARGUMENTS\n")
	write(t, filepath.Join(home, ".semantix", "commands", "hi.md"),
		"---\ndescription: home hi\n---\nH $ARGUMENTS\n")

	// Project hooks load automatically.
	write(t, filepath.Join(root, ".semantix", "settings.json"), `{
  "hooks": {
	...
	write(t, filepath.Join(root, "semantix-agent.toml"), `
[[plugins]]
name = "broken"
type = "stdio"
command = "definitely-not-a-real-binary-xyzzy-semantix"
auto_start = false
`)
	...
	r := capdiag.Collect(capdiag.Options{
		Root:            root,
		HomeDir:         home,
		SemantixHomeDir: filepath.Join(home, ".semantix"),
		Live:            false,
	})
	...
	mcp := r.MCP.Servers[0]
	if !mcp.Effective || mcp.Source != "project_config" || mcp.SourcePath != "<workspace>/semantix-agent.toml" {
	...
	r2 := capdiag.Collect(capdiag.Options{
		Root: root, HomeDir: home, SemantixHomeDir: filepath.Join(home, ".semantix"),
	})
...
func TestCollectUsesExactSemantixHomeForGlobalHooks(t *testing.T) {
	root := t.TempDir()
	home := t.TempDir()
	semantixHome := filepath.Join(home, "AppData", "Roaming", "semantix")
	write(t, filepath.Join(semantixHome, "settings.json"), `{
  "hooks": {
	...
	for _, source := range report.Hooks.Sources {
		if source.Scope == "global" {
			if source.Status != "ok" || source.HookCount != 1 {
				t.Fatalf("global hook source = %+v, want exact Semantix home settings", source)
			}
			if source.Path != "<semantix-home>/settings.json" && source.Path != "<semantix-home>\\settings.json" {
				// displayPath uses ToSlash
				if !strings.Contains(source.Path, "<semantix-home>") {
					t.Fatalf("global path = %q, want <semantix-home> prefix", source.Path)
				}
			}
			return
		}
...
func TestLoadForRootReadOnlyDoesNotRewriteTier(t *testing.T) {
	...
	t.Setenv("HOME", home)
	t.Setenv("SEMANTIX_HOME", filepath.Join(home, ".semantix"))
	userCfg := filepath.Join(home, ".semantix", "config.toml")
...
func TestPluginPackageCommandsAreReported(t *testing.T) {
	...
	pluginRoot := filepath.Join(semantixHome, "plugins", "demo")
	write(t, filepath.Join(pluginRoot, pluginpkg.NativeManifest), `{"apiVersion":"semantix.io/plugin/v2","name":"demo","commands":["commands"]}`)
