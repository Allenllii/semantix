...
func TestDocPathPrefersExisting(t *testing.T) {
	proj := t.TempDir()
	// An existing SEMANTIX.md should keep receiving notes (no split to AGENTS.md).
	if err := os.WriteFile(filepath.Join(proj, "SEMANTIX.md"), []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
	set := Load(Options{CWD: proj})
	if got := set.DocPath(ScopeProject); filepath.Base(got) != "SEMANTIX.md" {
		t.Errorf("should append to the existing SEMANTIX.md, got %s", got)
	}
