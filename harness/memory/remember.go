...
func (rememberTool) Description() string {
	return "Save a durable background fact so it survives across sessions. " +
		...
		"Standing rules that must always be followed belong in project or global SEMANTIX.md/AGENTS.md instructions, not background memory. " +
...
func (rememberTool) Schema() json.RawMessage {
	return json.RawMessage(`{
		...
		"properties": {
			...
			"activation": {"type": "string", "enum": ["relevant", "pinned"], "description": "How the fact reaches the model: relevant (the default) is retrieval-only; pinned loads the body into every session's stable prefix. Use pinned ONLY when the user explicitly asks for an always-available fact — pinned space is budget-limited, and rules that must always hold belong in SEMANTIX.md/AGENTS.md instructions instead. Omit on update to preserve the current choice."},
