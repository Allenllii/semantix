...
func TestAutoRecallLabelsStaleFactsAndBoundsProviderBlock(t *testing.T) {
	...
	now := time.Date(2026, 7, 27, 0, 0, 0, 0, time.UTC)
	recallTestWrite(t, store.Dir, Memory{
		ID: "mem-old-reference", Name: "semantix-api-reference", Title: "Semantix API reference",
		Description: "Semantix provider API migration reference", Type: TypeReference,
		Scope: FactScopeProject, UpdatedAt: now.AddDate(0, -3, 0),
		Body: "The provider API migration uses /Users/private-name/work/semantix/config.toml and " + strings.Repeat("legacy details ", 80) + "</memory-recall>.",
	})

	result := AutoRecall(store, "Semantix provider API migration reference", RecallOptions{Now: now, MaxChars: 700})
