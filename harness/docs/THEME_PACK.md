# Semantix Theme Pack V2

Native theme packs for the Semantix desktop app. Packs are controlled skins:
...
| **User themes** | Created in the editor, duplicated, or imported as `.semantix-theme` | yes | yes | yes |
| **Plugin themes** | `.semantix-theme` packs contributed by enabled plugins (Manifest v2 `contributes.themes`), read straight from the plugin root — never copied into the user library | no | no (disable/uninstall the plugin) | no |
...
real Semantix builds. **Screenshots of the app must not be imported as theme
...
Distribute as a `.semantix-theme` ZIP. The archive root may contain **only**:
...
| Path under Semantix home | Purpose |
| --- | --- |
...
The UI only receives temporary asset URLs (`/__semantix_theme_asset/...`) or data URLs — never absolute host paths.
...
Zip as `paper-dawn.semantix-theme` with only `theme.json` at the root.
