...
Semantix already has a solid extension kernel: `Contribution → RuntimeSnapshot`,
...
- Upgrade native plugins to `semantix.io/plugin/v2`
...
Native `semantix-plugin.json` requires:
...
- `semantix-agent plugin migrate <name> --to-v2` rewrites only pre-extension native
...
- `semantix-agent plugin doctor` reports dependency and protocol errors
...
| Manifest `semantix.io/plugin/v2` + reject v1 | Done |
