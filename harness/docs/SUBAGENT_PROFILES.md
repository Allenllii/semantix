...
with `runAs: subagent`: Semantix starts an isolated child agent, gives it the
...
```bash
semantix-agent subagent create reviewer \
...
`.semantix/skills/<name>/SKILL.md`; global profiles are stored under the
Semantix home Skill directory described in
...
printf '%s\n' 'Review the task and report only actionable findings.' | \
  semantix-agent subagent create reviewer --description "Code reviewer"
```

Names may contain letters, digits, `_`, `-`, and `.`. Semantix refuses a name
...
semantix-agent subagent try reviewer "review the current diff"
...
git diff | semantix-agent subagent run reviewer --max-steps 20
```
...
apply. Ordinary `semantix-agent run` remains a plain one-shot task entry point and
does not implicitly interpret `/<profile>` syntax.

## Manage profiles

```text
semantix-agent subagent list [--dir PATH]
semantix-agent subagent create <name> --description TEXT (--prompt TEXT | --prompt-file PATH)
  [--scope project|global] [--model REF] [--effort LEVEL]
  [--tools a,b] [--color NAME] [--dir PATH]
semantix-agent subagent edit <name> [--description TEXT]
  [--prompt TEXT | --prompt-file PATH] [--model REF] [--effort LEVEL]
  [--tools a,b] [--color NAME] [--dir PATH]
semantix-agent subagent delete <name> --yes [--dir PATH]
semantix-agent subagent try <name> [--model REF] [--max-steps N] [--dir PATH] <task>
semantix-agent subagent run <name> [--model REF] [--max-steps N] [--dir PATH] <task>
```
...
```bash
semantix-agent subagent edit reviewer --model= --effort= --tools= --color=
```
...
Profiles created in desktop settings and with `semantix-agent subagent create` share
...
`semantix-agent subagent list`, the current `--dir`, and `skills.disabled_skills`. If
...
invalid effort levels are rejected when Semantix resolves the effective model.
