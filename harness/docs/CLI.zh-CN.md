...
不带子命令运行 `semantix-agent` 会进入交互式终端界面。尚未配置 provider 时，先运行
`semantix-agent setup`。
...
```sh
semantix-agent upgrade                  # 安装最新正式版
semantix-agent upgrade --check          # 只报告目标版本
semantix-agent upgrade --force          # 重新安装当前正式版
```

更新器只选择严格的 `vX.Y.Z` 非 prerelease GitHub Release。1.x 兼容期内，旧渠道
位置参数与 `--channel` 仍可使用，但都会解析到同一正式版并打印废弃提示。历史
`[cli].update_channel` 值不再影响更新，并会在 Semantix 下次保存配置时移除。别名
`semantix-agent update` 的行为完全相同。

## 配置供应商

```sh
semantix-agent setup                    # 管理用户全局配置
semantix-agent setup --local            # 管理 ./semantix-agent.toml
semantix-agent setup /path/to/config.toml
```

在交互式终端中，`semantix-agent setup` 是一个暂存式供应商管理器。它会列出已配置的
provider，并支持：
...
在 CLI 与桌面端共用的 Semantix 全局 `.env` 中。如果变量名已被其他 provider 使用，
...
```sh
semantix-agent config currency             # 显示已保存值和最终解析结果
semantix-agent config currency auto        # 跟随解析后的 locale
...
`semantix-agent.toml` 无法覆盖，因此不支持 `--local`。自定义 provider 价格不会被修改。
...
```sh
semantix-agent config compact-ratio              # 查看生效值及来源
semantix-agent config compact-ratio 75           # 设置用户全局默认值
semantix-agent config compact-ratio --local 75   # 写入 ./semantix-agent.toml 项目覆盖
```

可设置范围为 65–85%，内置默认值为 80%。数值越低越早压缩，可能降低 prompt prefix
缓存复用率；数值越高则会在压缩前保留更多上下文。项目 `semantix-agent.toml` 的优先级高于
...
```sh
semantix-agent -p "总结这个仓库"
semantix-agent -p "总结这个仓库" --output-format json
semantix-agent run "实现 main.go 里的 TODO"
semantix-agent run --auto "实现 main.go 里的 TODO"
echo "解释这段代码" | semantix-agent run
```

未使用 `-p` 或结构化输出格式时，`semantix-agent run` 保持正常的终端流式展示。它也接受
...
```sh
semantix-agent run --ablate evidence,planner --metrics run.json "修复失败的测试"
```

这是测量工具，不是调优开关：关掉某个子系统只会让 Semantix 在它本来负责的工作上变差。
...
```sh
semantix-agent run --metrics run.json --trajectory run.trajectory.jsonl "修复失败的测试"
```
...
```sh
semantix-agent -p "列出有风险的改动" --output-format text
semantix-agent -p "总结 diff" --output-format json
semantix-agent run "运行测试" --output-format stream-json
```
...
`total_cost` 和 `currency`。如果一次结构化运行包含多种货币，Semantix 会直接报错，
不会输出容易误解的合计金额。
...
```sh
semantix-agent run --events-jsonl "运行 focused tests"
```
...
状态；首次使用脱敏机器接口时，Semantix 可能会在用户状态目录初始化一个私有身份密钥：
...
则解析指定项目的 session store；两者不能同时使用。都未指定时，Semantix 选择当前
...
机器 session ID 是带密钥的 opaque hash，不是 transcript 文件名。在同一个 Semantix
用户状态目录中，同一 session 的 ID 保持稳定；不同安装密钥会生成互不关联的 ID，无法再
根据时间戳或模型 label 离线猜测。迁移 Semantix 状态目录时，如果自动化依赖已有 machine
...
一次性运行可用 `semantix-agent run --resume QUERY "任务"`，支持 session 文件路径、
session ID，或来自 `--events-jsonl` / `semantix-agent session show --json` 的不透明
...
semantix-agent -p "运行指定测试" --allowed-tools "Bash(go test ./...)"
...
无人值守执行需要放行普通 writer fallback 时，使用 `semantix-agent run --auto ...`
...
在非交互运行（`semantix-agent run` / `-p`）下没有可应答的审批，各模式都以非阻塞方式解析。
...
semantix-agent -p "同时更新两个项目" \
  --add-dir ../frontend \
  --add-dir ../backend
```

相对路径从 workspace 根目录解析，并且必须是已存在的目录。Semantix 会解析符号链接、
...
bracketed-paste 动作（macOS 通常为 `Cmd+V`，其它平台使用终端自身配置）。Semantix 接管
本地会话的鼠标时，没有选区的右键会读取剪贴板文本并走同一粘贴路径，有选区时右键优先复制。
SSH 下远端进程无法读取本机剪贴板，请使用终端粘贴快捷键；`/mouse` 可恢复终端原生右键菜单。
图片粘贴由 Semantix 接管：macOS/Linux 使用 `Ctrl+V`，Windows 使用 `Alt+V`，也可运行
...
| `/semantix:docs [问题]` | 当已有自定义命令或兼容插件/Skill 别名占用 `/docs` 时优先使用的内置后备入口；若这个名称也已被占用，菜单会选择下一个空闲的 `semantix:` 限定名，不覆盖原命令。 |
...
这些命令始终作用于当前 session controller。当会话位于远端主机上（`semantix-agent remote
