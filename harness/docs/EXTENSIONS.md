...
Extensions let a plugin package change what Semantix does at runtime —
...
```bash
semantix-agent plugin install git:github.com/owner/extension --dry-run   # preview
semantix-agent plugin install git:github.com/owner/extension --yes       # install
semantix-agent plugin show <name>                                        # details
semantix-agent plugin doctor <name>                                      # validate
```
...
vendor method `_semantix.io/session/reloadExtensions`:
...
2. When idle, Semantix starts new sidecars and builds a new runtime
   snapshot.
...
When runtimes are installed, Semantix initializes at most four sidecars at once
...
1. Add `apiVersion: "semantix.io/plugin/v2"` to `semantix-plugin.json` and
...
3. Build the runtime binary, preview its trust and capabilities with
   `semantix-agent plugin install /path/to/plugin --dry-run`, then install it with
   `--link --yes`.
4. Validate with `semantix-agent plugin doctor <name>`, run `/reload` while idle,
...
- Native `semantix-plugin.json` manifests must declare the exact
  `semantix.io/plugin/v2` API version. There is no v1 dual-read or automatic
  migration path because extension manifests were not publicly released on v1.
- Older Semantix versions ignore extension-only state: the per-session
  ...
  clients without `semantix.extensionSurface` get text fallbacks).
- `plugin-packages.json` keeps its existing schema; an enabled installed
  runtime *is* the trust record.

## Security model

A code extension runs outside the Semantix sandbox with the unfiltered
...
- a crashed sidecar fails its own operations explicitly — Semantix never
