# Semantix Plugin Packages

Semantix plugin packages bundle skills, hooks, MCP servers, prompts, themes,
and code extensions behind one installable unit.

## CLI Mode

Use `semantix-agent plugin` when installing or managing plugin packages from a
terminal. Plugin packages are installed globally under the Semantix home
directory.
...
- A local directory that contains `semantix-plugin.json`,
  `.codex-plugin/plugin.json`, or `.claude-plugin/plugin.json`.

Preview the install plan without writing files:

```bash
semantix-agent plugin install git:github.com/obra/superpowers --dry-run
```

Install a plugin after reviewing the plan:

```bash
semantix-agent plugin install git:github.com/obra/superpowers --yes
```

Install with an explicit name or replace an installed plugin with the same name:

```bash
semantix-agent plugin install git:github.com/obra/superpowers --name superpowers --replace --yes
```

Use a local directory in developer mode:

```bash
semantix-agent plugin install /path/to/plugin --link --replace --yes
```
...
- `--link` links a local plugin directory instead of copying it into Semantix's
  plugin storage. Moving or deleting that directory breaks the linked plugin.

Running `semantix-agent plugin install <source>` without `--dry-run` or `--yes`
...
```text
~/.semantix/plugin-packages.json
~/.semantix/plugins/<name>/
```
...
```bash
semantix-agent plugin doctor superpowers
```
...
# Agent:   /semantix-guide
```

Enable or disable a plugin without uninstalling it:

```bash
semantix-agent plugin disable superpowers
semantix-agent plugin enable superpowers
```

Remove a plugin:

```bash
semantix-agent plugin remove superpowers --yes
```
...
Semantix loads its capabilities into normal interactive sessions:
...
- **MCP servers** join the normal MCP/tool flow. Ask for the task you want done;
  Semantix can call the plugin's tools when they are relevant.

After installing, enabling, disabling, or updating a plugin from a separate
terminal while a session is already running, start a new `semantix-agent` session or
...
- **Developer mode: link source folder** appears for **Local folder** installs.
  It links the selected directory instead of copying it into Semantix's plugin
...
Semantix plugins can declare `semantix-plugin.json` at the plugin root:
...
Relative paths are resolved inside the plugin root. Semantix does not run
...
- Existing native hooks that declare neither field keep the historical
  Semantix shell-command behavior. `shellCommand: true` remains supported as
  the legacy spelling of shell form.

## Manifest v2 (Extensions)

Native Semantix extensions use the exact v2 `apiVersion`:
...
```json
{
  ...
  "runtime": {
    "command": "${SEMANTIX_PLUGIN_ROOT}/bin/example",
    "args": [],
    "env": {},
    "required": true,
    "priority": 0,
...
- Native `semantix-plugin.json` manifests must declare the exact
  `semantix.io/plugin/v2` value. v1 and missing versions are rejected; there
...
- v2 resource discovery is explicit. Semantix loads only the skills, agents,
  ...
  implicitly.
- Minor aliases (`semantix.io/plugin/v2.0`, `v2.1`, …) and unknown major
...
- `prompts` are prompt templates using the same semantics and argument
  ...
  a compatible alias.
- `themes` are `.semantix-theme` files shown read-only in Desktop Settings as
...
The `runtime` block declares a code extension — a sidecar process Semantix
...
- `command`/`args`/`env` are **exec form only** — the command is the
  executable (never shell-interpreted); `${SEMANTIX_PLUGIN_ROOT}` expands to
...
`semantix-agent plugin show`, capability diagnostics, and the Desktop installer
...
Semantix also reads Codex plugin manifests at `.codex-plugin/plugin.json` and
...
Semantix construct; it does not by itself guarantee every runtime decision an
...
Superpowers and Claude-style skill packs, Semantix maps the following
compatibility conventions. Native v2 manifests use only their explicit
declarations and do not apply these fallbacks:

- `skills` to Semantix skill roots. A Claude manifest that declares no
...
- `commands/` (and `.claude/commands/`) to Semantix custom slash commands: each
  ...
  details report that conflict. Native `semantix-plugin.json` manifests can
...
- `agents/*.md` to manually invoked, plugin-owned subagent profiles. Claude
  model aliases inherit the active Semantix model; inline `tools` lists map to
  Semantix tool names, including wildcard MCP names such as `mcp__*__search`.
  Agents use `/<plugin>:agent:<name>`, so an upstream agent and skill may share
  the same name without shadowing one another.
- `hooks/session-start-codex` to the Semantix `SessionStart` hook when present.
- For Codex compatibility packages, a plugin-root `CLAUDE.md` file to a built-in
  `SessionStart` context hook. The file is read directly by Semantix, without
  spawning a shell command. Claude plugin manifests ignore this file, matching
  Claude Code's plugin contract.
- `.claude/settings.json` and `hooks/hooks.json` command hooks to Semantix hook
  ...
  translated between Semantix's own tool names and Claude's (`bash` ↔
  `Bash`, `write_file` ↔ `Write`, ...), so a matcher like `"Bash"` fires
  correctly; every Semantix subagent-spawning tool (`task`, `read_only_task`,
  ...
  includes Claude's required `prompt` and `description`; Semantix supplies a
  stable operation label when its tool call omitted the optional description.
  `tool_input` keys that
  Semantix names differently from Claude are renamed too — `path` becomes
  ...
  task content. `NotebookEdit` also supplies `new_source` from Semantix's
  ...
  interrupted}` shape (Semantix combines both streams into `stdout`; the
  ...
  expands `${CLAUDE_PLUGIN_ROOT}` and `${SEMANTIX_PLUGIN_ROOT}` (plus their
  ...
  PowerShell-fallback selection as Semantix's shell tool; when a hook points to
  ...
  explicit Bash hooks. `semantix-agent plugin doctor <name>` and
  `semantix-agent doctor capabilities` report a missing required shell before the
  ...
  `Stop`/`SubagentStop` hook (which cannot block the turn in Semantix), or a
  matcher that covers one of three inputs Semantix cannot losslessly express:
  `WebFetch.prompt`, `NotebookEdit.cell_id` for a Semantix `cell_number` call,
  or `TaskOutput.task_id` when Semantix `wait` covers multiple/all jobs. Each
...
Unsupported Claude hook item types are skipped with a warning. Semantix does not
run third-party install scripts.

Plugin hooks receive these environment variables:

- `SEMANTIX_PLUGIN_ROOT`
- `SEMANTIX_PLUGIN_NAME`
- `SEMANTIX_PLUGIN_VERSION`
- `SEMANTIX_HOME`
- `SEMANTIX_WORKSPACE_ROOT`
- `CLAUDE_PROJECT_DIR`
- `CLAUDE_PLUGIN_ROOT`
