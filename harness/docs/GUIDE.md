...
Resolution order: **flag > `./semantix-agent.toml` > the user config file >
built-in defaults**. Starting with **Semantix v1.8.1**, the user config lives at
`~/.semantix/config.toml` on macOS/Linux and
`%AppData%\semantix\config.toml` on Windows; see
[Configuration paths](./CONFIG_PATHS.md) for migration and related data paths.
Fields marked user/global only are not overridden by `./semantix-agent.toml`.
Provider entries name secrets with `api_key_env`, while the secret values live in
Semantix's global `<Semantix home>/.env`, shared by CLI and desktop. Project
...
provider keys or Semantix control variables. See
...
# language    = "zh"               # ui language; empty = auto-detect from $LANG / $SEMANTIX_LANG
...
# password_hash = ""           # bcrypt hash generated with semantix-agent serve --hash-password --password '...'
# behind_proxy = false         # true only behind a trusted reverse proxy

[[plugins]]
name    = "example"
command = "semantix-plugin-example"
...
Most day-to-day settings belong in `config.toml` or the global Semantix `.env`
described above. The variables below are process-level advanced switches; set
them before launching Semantix. Project `.env` files are not a runtime source for
Semantix control variables.

### CLI telemetry

The CLI can send a once-per-day anonymous active-install ping and bounded,
content-free event counters to `https://crash.semantix.ensureok.ai`. Configure the
user-global policy with:

```bash
semantix-agent config telemetry          # print the effective mode
semantix-agent config telemetry auto     # default: local interactive TTY only
semantix-agent config telemetry on       # also allow local headless `semantix-agent run`
semantix-agent config telemetry off      # disable and delete pending counter files
```

On the first eligible release-build interactive session, Semantix explains the
...
`DO_NOT_TRACK` is set or `SEMANTIX_TELEMETRY=0`. Under `auto`, redirected/piped
...
Semantix never uploads prompts, answers, reasoning, tool names/arguments/output,
...
`<Semantix home>/cli-crash-reports`. Semantix keeps at most 10 files with owner-only
...
```bash
semantix-agent report                 # preview newest; prompt before sending on a TTY
semantix-agent report list            # list local reports
semantix-agent report show [ID]       # preview without sending
semantix-agent report send [ID]       # explicit send; delete locally only after success
semantix-agent report delete [ID]     # delete without sending
```

Piped or redirected `semantix-agent report` calls only preview and never prompt or
...
For local use, `semantix-agent web` starts the browser UI and opens it in your default
...
Use `semantix-agent web --no-open` when you want to start the foreground Web server
and print its URL without opening a browser tab. The lower-level
`semantix-agent serve` command starts the same engine without opening a browser by
...
`semantix-agent web` starts at `127.0.0.1:8787`, automatically tries 8788, 8789, and
...
single-writer heartbeat file under `<Semantix home>/server/instances/`; clean
...
Semantix home without overwriting registry state. The process stays attached to
the terminal; stop it with Ctrl-C.

An explicit `semantix-agent web --auth none` disables the default token and should be
used only when the listener is intentionally trusted. `semantix-agent serve` keeps its
...
```bash
semantix-agent serve --auth token
semantix-agent serve --addr 0.0.0.0:8787 --auth token
semantix-agent serve --auth password --password 'temporary-password'
```
...
```bash
semantix-agent serve --hash-password --password 'strong-password'

# <Semantix home>/config.toml
[serve]
...
connect. After authentication, enter the key there; Semantix writes it to this
...
`semantix-agent acp` exposes Semantix as an ACP v1 stdio agent for editors and other
...
MCP servers, permission requests, and the Semantix mid-turn steering extension.

## Remote SSH

The remote module runs Semantix on a remote host and reaches it over your own
SSH connection — VS Code Remote-SSH style. It bootstraps a persistent headless
`semantix-agent serve` on the remote host, forwards a local loopback port to it, and
...
`[secrets]`, a project `semantix-agent.toml` cannot inject or override remote hosts —
a cloned repo can never steer where Semantix opens SSH connections. Credentials
follow the provider idiom: the host names an env var (`passphrase_env`,
`password_env`) whose value lives in Semantix's global `.env`; key material
...
```bash
semantix-agent remote add gpu-box dev@203.0.113.7 --workspace '~/projects/app'
semantix-agent remote import --all              # import aliases; ssh -G resolves Include/Match rules when connecting
semantix-agent remote test gpu-box              # dial + auth + host-key confirmation
semantix-agent remote connect gpu-box --open    # bootstrap serve, tunnel, open the URL
semantix-agent remote serve status gpu-box
semantix-agent remote fs ls gpu-box:'~/projects/app'
```
...
plus a Semantix-managed `~/.semantix/remote/known_hosts`. A first-seen key
...
Remote-side state lives under the remote host's `~/.semantix/remote/`:
...
remote Semantix credential file, and activates the Provider without restarting
...
for both protocols. On startup, Semantix upgrades unmodified legacy
...
redundant preset is no longer offered for new access. Semantix can prefill editable custom-provider entries for Kimi CN,
Kimi Global,
...
usually needs only the provider API key: the key value is stored in Semantix home
...
Semantix preserves complete assistant messages across turns, sends output limits
...
chat path. In this mode Semantix previews and sends chat requests to:
...
example `https://gateway.example.com/v1/chat/completions`. Semantix then sends
...
| Extra request body | A JSON object merged into the top-level chat request body. | Use only for provider-specific flags such as `{"enable_thinking": true}`. Semantix still owns core fields such as `model`, `messages`, `tools`, `stream`, and `thinking`, and null values are rejected. |
...
| Model capability mode | Which reasoning request protocol Semantix should use for this provider. | Keep **Auto-detect** unless the gateway is misdetected or the model docs require a specific reasoning format. |
...
| Context window | The provider-wide token budget Semantix uses for automatic context cleanup. `0` disables automatic compaction. | Set it to the provider's model context limit; use a per-model override below when selected models differ. |
...
| Option | Effect |
| --- | --- |
| Auto-detect (recommended) | Semantix chooses the request shape from model capability metadata and endpoint detection. |
...
| Option | Effect |
| --- | --- |
| Auto (provider default) | Does not write an explicit provider-level `thinking` override. Semantix uses the provider/model default behavior. |
...
`extra_body` is merged into the chat JSON request body. Semantix keeps core
...
`hookSpecificOutput.additionalContext`; Semantix injects that text once into the
...
`.claude/settings.json` command hooks are also mapped to matching Semantix hook
events.
...
row, press a new key combination, and Semantix saves it for the desktop app.
...
| `Cmd+Z` on macOS, `Ctrl+Z` on Windows/Linux | Undoes the latest composer edit | Native typing stays in the WebView history; Semantix-managed paste, cut, folded blocks, and structured tokens are restored as complete transactions. |
...
while `/work` command arguments remain the stable English identifiers.
...
| Right-click with no active selection | Pastes clipboard text locally | In a local session with in-app mouse capture on, Semantix reads text only and routes it through the normal bracketed-paste handling. Over SSH, use the terminal paste shortcut because the remote process cannot read the local clipboard; `/mouse` restores the terminal's native right-click menu. Right-click with an active selection still copies that selection. |
| `/mouse` | Toggles in-app mouse capture | Off hands the mouse back to your terminal, restoring its native click-drag selection and right-click context menu, at the cost of in-app drag-select, the transcript scrollbar, and wheel-scroll. Set `SEMANTIX_DISABLE_MOUSE=1` to start every session with it off. |
...
| Your terminal's text-paste shortcut | Pastes text | Text stays on the terminal's bracketed-paste path (`Cmd+V` on macOS, commonly `Ctrl+Shift+V` on Linux, and the terminal's configured shortcut elsewhere). Semantix consumes the resulting paste event and never probes for an image first. |
...
| `/work [light|balanced|delivery]` | Shows or switches the current session's work mode | `/preset`, `/work-mode` and `/profile` are compatibility aliases. Switching happens in place — no controller rebuild — preserves the conversation and approval posture, and is blocked while work is active. |
| `/theme [auto|light|dark|style]` | Shows or switches the CLI theme | Bare `/theme` lists background modes and named accent palettes. The choice is saved to the user config; `SEMANTIX_THEME` and `SEMANTIX_THEME_STYLE` can override it for one run. |
...
`semantix-agent` can grant Bash as an exact command or as a conservative command
...
`semantix-agent run --auto ...`, `-y`, or `--permission-mode auto` when unattended
...
Semantix always removes saved provider and bot credential variables from tool
...
`/tmp`). No user setup is required: Semantix automatically exports `TMPDIR`,
...
Semantix-generated and project scripts should use the standard temporary
...
**Windows note:** Semantix does not ship an OS-level Bash sandbox on Windows.
The effective mode is fixed to `off`; even an older config containing
`bash = "enforce"` resolves to `off`, `semantix-agent doctor` flags the ignored value,
...
For coding-quality reports, run `semantix-agent doctor quality <branch-id-or-path>`
...
or Discussion. This differs from `semantix-agent doctor session`, whose support zip
...
semantix-agent doctor capabilities

# Machine-readable (stdout is pure JSON)
semantix-agent doctor capabilities --json

# Another workspace root
semantix-agent doctor capabilities --root /path/to/project

# Live MCP probe — only when you explicitly allow starting third-party servers
semantix-agent doctor capabilities --live --timeout 5s
```

| Surface | How |
| --- | --- |
| CLI | `semantix-agent doctor capabilities` (above) |
...
| Agent | `/semantix-guide` (built-in inline skill) or ask naturally; it prefers static doctor JSON before `--live` |

Exit code `0` allows warnings/info; `1` means at least one `error` (or a live
start failure); `2` is bad flags. This is separate from `semantix-agent doctor`
(providers/sandbox) and `semantix-agent plugin doctor <name>` (one package).

## Plugins (MCP)

Semantix is an MCP client. A `[[plugins]]` entry's `type` selects the transport:
...
`semantix-agent mcp auth <name>` in the CLI, or click **Sign in** for that server in
the Desktop MCP panel. Semantix performs OAuth metadata discovery, dynamic
client registration, PKCE S256 authorization, and refresh-token
rotation. Discovery and token requests use the same Semantix network-proxy
settings as the MCP connection.

OAuth client and token state is kept outside the workspace in the server's
private Semantix state directory, written with mode `0600`, and bound to the
full configured resource URL. An explicit static `Authorization` header always
takes precedence. **Clear authentication** removes only Semantix's local OAuth
state; it does not sign out the third-party browser session. Semantix opens the
...
registry**, or use `semantix-agent mcp browse [query]` and
`semantix-agent mcp install <registry-name>`. Registry access is explicit and never
...
connect**, `/mcp add`, or ask Semantix to install a package or URL. These
...
project's `semantix-agent.toml` or `.mcp.json` remain in that project and are trusted
...
same-name global installs; inside a project, `semantix-agent.toml` overrides
...
Semantix deliberately trusts an installed server to describe those hints
...
loading older files and removed the next time Semantix saves that MCP entry.
...
exposes. `make build` also produces `bin/semantix-plugin-example` — a runnable
...
[Capability diagnostics](./CAPABILITY_DIAGNOSTICS.md)
(`semantix-agent doctor capabilities` or **Settings → Diagnostics**).
...
**Already have an `.mcp.json`?** Drop it in the project root and Semantix
reads it as-is — the `mcpServers` spec (`command`/`args`/`env`, `type`/`url`/
`headers`, `${VAR}` expansion) maps field-for-field onto `[[plugins]]`. Both
sources are merged; on a name collision `semantix-agent.toml` wins.
...
**Upgrading from `0.x`?** Your old `~/.semantix/config.json` is still read for its
`mcpServers` (honouring `mcpDisabled`) as a lowest-priority source, so MCP servers
keep working — move them into `semantix-agent.toml`'s `[[plugins]]` or a `.mcp.json` when
convenient.

## Slash commands

In an interactive `semantix-agent` session, built-in commands (`/compact`, `/context`, `/new`, `/clear`, `/rewind`,
`/tree`, `/branch`, `/switch`, `/todo`, `/model`, `/work`, `/mcp`, `/skills`, `/hooks`,
...
`/explore`, `/test`, and `/semantix-guide` also appear in the slash menu and via
`run_skill` (bodies load on demand; only the index line is cache-stable). Use
`/semantix-guide` when you need config or capability troubleshooting; it points
at `semantix-agent doctor capabilities` (see
...
Markdown files under `.semantix/commands/` (project) or `~/.semantix/commands/`
...
one with `/<name> <task>`; Semantix runs an isolated child loop and keeps only
...
changing the ordinary `semantix-agent run` task semantics:

```bash
semantix-agent subagent list
semantix-agent subagent create reviewer --description "Review changes" --prompt-file reviewer.md --tools read_file,grep,bash
semantix-agent subagent edit reviewer --effort high --model deepseek-pro
semantix-agent subagent try reviewer "review the current diff"   # always read-only
semantix-agent subagent run reviewer "review and fix the current diff"
semantix-agent subagent delete reviewer --yes
```
...
- **Standing instructions** come from hierarchical `SEMANTIX.md`, `AGENTS.md`,
...
Semantix automatically recalls a small set of relevant facts before each real
...
Semantix bundles the Markdown files from `docs/` and the reviewed
...
`changelog/v0.5.1.md` and `changelog/v0.5.1.zh-CN.md`, so questions about a
specific version, upgrades, fixes, or known risks work offline. The agent should
use the tool before web search or assumptions when a question concerns Semantix
...
`/docs 0.5.1 changelog`) to make Semantix search the corpus locally first and
...
expose the built-in corpus as `/semantix:docs` instead. If that qualified name is
also already owned, Semantix selects the next free `semantix:`-qualified fallback
...
Goal is the unified runtime for long-running objectives. Semantix keeps working
...
bounded Goal evaluator. Legacy `.semantix/autoresearch/<task-id>/` archives are
...
Embed `@` references in a message and Semantix resolves them before sending, as
...
`semantix-agent setup` manages providers, model lists, credentials, connection tests,
...
The planner sees loaded `SEMANTIX.md` / `AGENTS.md` memory and a small read-only
...
Semantix routes each turn deterministically without another classifier model:
...
Semantix manages normal execution automatically: if an active todo produces no
...
| `semantix-agent review` (CLI) | Read-only review of a diff or branch |
...
`--profile economy|balanced|delivery` (for example, `semantix-agent run --profile
...
forcing a write.
Inside an interactive TUI session, use `/work` to inspect the current
choice or `/work light|balanced|delivery` to switch it. `/preset`, `/work-mode`
and `/profile` are compatibility aliases. The switch happens in place — no
controller rebuild — preserving history, the session path, leases, and the
Ask/Auto/YOLO posture; it is rejected while a turn, approval/question, background
job, or another runtime switch is active. This
...
`Shift+Tab` in the CLI. Semantix first drafts a plan, then waits for approval
...
session, or `semantix-agent config reasoning-language auto|zh|en` in a shell/script.
Pass `--local`
