...
优先级：**flag > `./semantix-agent.toml` > 用户配置文件 > 内置默认值**。从
**Semantix v1.8.1** 开始，用户配置位于 macOS/Linux 的
`~/.semantix/config.toml`，Windows 为 `%AppData%\semantix\config.toml`；迁移和相关数据路径见
[配置路径](./CONFIG_PATHS.zh-CN.md)。标注为“仅用户/全局”的字段（包括 agent 轮数上限）不会被 `./semantix-agent.toml` 覆盖。
Provider 通过 `api_key_env` 命名密钥，真实密钥值保存在 CLI 与桌面端共用的
Semantix 全局 `<Semantix home>/.env`。项目 `.env`、home `.env`、继承的 shell 环境变量、旧 credentials 和系统 keyring 都不再作为 provider key 的运行时 fallback；旧凭据只作为迁移来源读取。项目 `.env` 仍会作为当前 workspace 范围内的 MCP/plugin 非 provider `${VAR}` 展开来源，但不会导入 provider key 或 Semantix 控制变量。全局 `config.toml` 和 `.env` 的完整结构见
[配置路径](./CONFIG_PATHS.zh-CN.md)。
...
插件包可通过 `hooks/session-start-codex` 或插件根目录 `CLAUDE.md` 提供该启动上下文；Claude 风格 `.claude/settings.json` command hooks 也会按同名事件映射到 Semantix hooks。

```toml
default_model = "deepseek-flash"   # 执行器；设 [agent].planner_model 可加规划器
# language    = "zh"               # 界面语言；为空则按 $LANG / $SEMANTIX_LANG 自动检测
...
# password_hash = ""           # 用 semantix-agent serve --hash-password --password '...' 生成
...
多数日常设置应写在 `config.toml` 或前文提到的 Semantix 全局 `.env` 中。下面这些变量是进程级高级开关；
需要在启动 Semantix 之前设置。项目 `.env` 不是 Semantix 控制变量的运行时来源。

### CLI 上报统计

CLI 可以向 `https://crash.semantix.ensureok.ai` 发送每日最多一次的匿名活跃安装 ping，
以及有界、完全不含内容的事件计数。使用以下用户全局命令配置：

```bash
semantix-agent config telemetry          # 查看当前生效模式
semantix-agent config telemetry auto     # 默认：仅本机交互式 TTY
semantix-agent config telemetry on       # 也允许本机 headless `semantix-agent run`
semantix-agent config telemetry off      # 关闭并删除待发送计数文件
```
...
`SEMANTIX_TELEMETRY=0` 也会关闭。`auto` 模式下，重定向、pipe 或其他非交互会话
...
Semantix 绝不会上传 prompt、回答、reasoning、工具名/参数/输出、路径、仓库/分支、
session ID、精确 token/费用、Provider/model 名称、base URL 或环境变量。

### CLI 崩溃报告

当未处理的 Go panic 到达 CLI 入口调用栈时，Semantix 会把脱敏报告保存在
`<Semantix home>/cli-crash-reports`。最多保留 10 份，文件权限仅限当前用户读取。
...
```bash
semantix-agent report                 # 预览最新报告；TTY 中询问后才发送
semantix-agent report list            # 列出本地报告
semantix-agent report show [ID]       # 仅预览，不发送
semantix-agent report send [ID]       # 明确发送；成功后才删除本地副本
semantix-agent report delete [ID]     # 不发送，直接删除
```

通过 pipe 或重定向运行 `semantix-agent report` 时只会预览，不会询问或发送。CLI telemetry
设置不会自动发送或自动删除这些
...
本机使用时，`semantix-agent web` 会启动浏览器 UI，并自动用默认浏览器打开。也可以在 CLI 交互会话中
执行 `/web`：Semantix 会保存当前会话、恢复终端，然后打开明确的
...
`semantix-agent web --no-open`。底层的 `semantix-agent serve`
默认不会打开浏览器，继续用于远程开发机、进程托管、tunnel、反向代理和需要认证分享的场景。

`semantix-agent web` 从 `127.0.0.1:8787` 开始监听；端口占用时会依次尝试 8788、8789……，
最多递增重试 100 次。它默认启用自动生成的 Token，即使配置中的 `[serve].auth_mode`
是 `none` 也一样。每个运行实例都会在 `<Semantix home>/server/instances/` 下写入自己的
单写者 heartbeat 文件；正常退出时只删除自己的文件，新实例则会惰性清理已确认进程死亡的记录。
因此多个 Web 实例可以共用同一个 Semantix home，而不会相互覆盖登记状态。服务保持在前台运行，
按 Ctrl-C 停止。

显式传入 `semantix-agent web --auth none` 可以关闭默认 Token，只应在监听地址确定可信时使用。
`semantix-agent serve` 则保持向后兼容：默认监听 `127.0.0.1:8787`，认证模式仍由配置决定，空配置为
...
配置页，而不是在浏览器连接前直接失败。通过 Serve 认证后可在该页输入 Key；Semantix 会以受限
...
`semantix-agent acp` 把 Semantix 作为 ACP v1 stdio agent 提供给编辑器和其他 host 客户端。
独立的 **[ACP 编辑器接入](./ACP.zh-CN.md)** 文档集中说明启动方式、能力协商、会话生命周期、
彼此独立的模型/工作/协作/审批控制轴、客户端文件与 terminal 能力、MCP server、权限请求，
以及 Semantix 的回合中引导扩展。

## 远程 SSH

远程模块让 Semantix 在远端主机上运行,并通过你自己的 SSH 连接访问它 —— 即 VS Code
Remote-SSH 式的体验。它在远端主机上引导一个常驻的 headless `semantix-agent serve`,把本地一个
...
`semantix-agent.toml` 无法注入或覆盖远程主机 —— 克隆的仓库永远无法左右 Semantix 向何处发起 SSH
连接。凭据沿用 provider 惯例:主机只记录环境变量名(`passphrase_env`、`password_env`),其值
存放在 Semantix 全局 `.env` 中;私钥内容本身从不存储 —— `identity_file` 只是路径。
...
semantix-agent remote import --all              # 导入别名；连接时通过 ssh -G 解析 Include/Match 等规则
semantix-agent remote test gpu-box              # 拨号 + 认证 + 主机密钥确认
semantix-agent remote connect gpu-box --open    # 引导 serve、建隧道、打开 URL
...
主机密钥会对照你的 OpenSSH `~/.ssh/known_hosts`(只读)以及 Semantix 托管的
`~/.semantix/remote/known_hosts` 校验。首次见到的密钥会提示 TOFU 确认并记入托管文件;与已记录
密钥冲突的密钥会硬失败并指明出错的行,绝不自动接受。

远端侧状态位于远端主机的 `~/.semantix/remote/`:`serve-<工作区 slug>.json`(pid、绑定的回环
...
Remote SSH 的独立 Semantix 原生窗口。主窗口持有 SSH 隧道；远程窗口是隔离的轻量外壳，不会恢复
或抢占本地对话会话。远程网页使用**远端**主机上的 Provider 配置与 API Key —— 桌面端绝不会把
本机 Provider 暴露给远端主机。如果远端缺少当前 Provider 的 API Key，窗口会先显示经过认证的
配置页，只把 Key 保存到远端 Semantix 凭据文件，并在不重启远端 Serve 的情况下激活 Provider。
...
`DEEPSEEK_API_KEY`。启动时，Semantix 会自动升级仍使用官方端点、标准密钥和标准模型设置且
...
接入不再展示这个重复预设。Semantix 还可以预填以下可编辑的自定义 provider：
...
预设路径通常只需要填写服务商 API Key：真实 key 会写入 Semantix home `.env`，
...
上下文窗口以及官方 `low`/`high`/`max` 推理强度（默认 `max`）。对官方 K3 端点，Semantix
...
**API 地址** 填写服务端点。默认模式下，Semantix 会预览并把聊天请求发送到：
...
开启 **完整 URL**。开启后 Semantix 会直接使用该地址，不再追加 `/chat/completions`。
输入框下方的预览就是最终请求地址。
...
| 字段 | 作用 | 什么时候改 |
| --- | --- | --- |
| `api_key_env` | 该 provider 使用的 API key 环境变量名。桌面端保存的真实 key 会写入 Semantix home `.env` 的同名变量；TOML 配置里只保存变量名。 | 多个 provider 需要不同 key 时改名；服务不需要 API key 时可以留空。 |
...
| 额外请求体 | 合并到聊天请求体顶层的 JSON 对象。 | 仅用于服务商专用开关，例如 `{"enable_thinking": true}`。`model`、`messages`、`tools`、`stream`、`thinking` 等核心字段仍由 Semantix 控制，且不接受 `null` 值。 |
...
| 模型能力模式 | 指定 Semantix 对该 provider 使用哪种 reasoning 请求协议。 | 默认用“自动识别”。只有网关被误判，或模型文档要求特定 reasoning 格式时再切换。 |
...
| 上下文窗口 | Semantix 用于自动清理上下文的 provider 级 token 预算。`0` 表示禁用自动 compaction。 | 按该 provider 的模型上下文上限填写；所选模型规格不同时使用下方的逐模型覆盖。 |

每个已选模型还提供一个可选的 **上下文窗口** 输入框。留空时继承 provider
级设置；填写正整数时只覆盖该模型。这样，同一端点下的长上下文模型不会过早
compaction，短上下文模型也不会在 Semantix 清理前被服务端拒绝。
...
| 选项 | 作用 |
| --- | --- |
| 自动识别（推荐） | Semantix 根据模型能力元数据和端点自动选择请求格式。 |
...
| 选项 | 作用 |
| --- | --- |
| Auto（使用服务默认） | 不写 provider 级 `thinking` 覆盖，让 Semantix 使用 provider/model 默认行为。 |
...
桌面端快捷键在 **设置 → 快捷键** 中管理。选择可配置的行后按下新的组合键，Semantix 会为桌面端保存该绑定。
...
| macOS `Cmd+Z`，Windows/Linux `Ctrl+Z` | 撤销输入框中的最近一次编辑 | 普通键入继续由 WebView 原生历史管理；Semantix 接管的粘贴、剪切、折叠块和结构化 token 会作为完整事务恢复。 |
...
| 没有活动选区时右键 | 在本地会话粘贴剪贴板文本 | 本地会话开启鼠标接管时，Semantix 只读取文本并交给正常的 bracketed-paste 处理。SSH 下远端进程无法读取本机剪贴板，请使用终端粘贴快捷键；`/mouse` 可恢复终端原生右键菜单。存在活动选区时，右键仍优先复制该选区。 |
| `/mouse` | 切换应用内鼠标接管 | 关闭后由终端处理原生拖选和右键菜单，但会失去应用内选区、滚动条和滚轮。可用 `SEMANTIX_DISABLE_MOUSE=1` 让每次会话默认关闭。 |
| `Ctrl+C` | 复制、取消、清空或退出 | 有 transcript 或输入框活动选区时优先复制；否则取消运行中的 turn、清空非空输入，或在空输入下连按两次退出。 |
| `Ctrl+D` | 退出 TUI | 立即退出。 |
| 终端的文本粘贴快捷键 | 粘贴文本 | 文本保持终端原生 bracketed-paste 路径：macOS 通常是 `Cmd+V`，Linux 通常是 `Ctrl+Shift+V`，其它环境使用终端自身配置。Semantix 只消费收到的文本粘贴事件，不会先探测图片。 |
...
Semantix 始终会从工具子进程环境中移除已保存的 provider 与 bot 凭据变量，并自动把
...
`/tmp`）。用户不需要设置：Semantix 会自动为 Bash 和客户端托管的 ACP 终端注入
...
Semantix 生成的脚本和项目脚本应使用标准临时目录变量，不要硬编码 `/tmp`；用户无需
自行设置这些变量。例如：
...
**Windows 说明：**Semantix 不在 Windows 上提供 OS 级 Bash 沙箱，生效模式固定为
`off`。旧配置即使写了 `bash = "enforce"` 也会解析为 `off`，`semantix-agent doctor`
...
反馈编码质量问题时，可运行 `semantix-agent doctor quality <branch-id-or-path>`（加
...
或 Discussion。它不同于 `semantix-agent doctor session`：后者生成的支持 zip 含完整未脱敏
会话，只能在可信支持渠道分享。
...
| 入口 | 用法 |
| --- | --- |
| CLI | 见上方 `semantix-agent doctor capabilities` |
| 桌面端 | **设置 → 诊断** — 刷新、复制脱敏 JSON、可选「包含当前会话运行状态」（只读活动标签 Host，**不**启动 MCP） |
| Agent | `/semantix-guide`（内置 inline Skill）或自然语言描述症状；优先静态 doctor JSON，再问是否 `--live` |

退出码：`0` 允许 warning/info；`1` 表示存在 `error`（或 live 启动失败）；`2` 为参数错误。与 `semantix-agent doctor`（provider/沙箱）以及 `semantix-agent plugin doctor <name>`（单个插件包）相互独立。

## 插件（MCP）

Semantix 是一个 MCP 客户端。`[[plugins]]` 的 `type` 选择传输：`stdio`（默认）启动本地子进
...
CLI 可运行 `semantix-agent mcp auth <name>`，桌面端则在 MCP 面板点击该 server 的 **登录**。
Semantix 会执行 OAuth 元数据发现、动态客户端注册、PKCE S256 授权与
refresh token 轮换；发现和 token 请求与 MCP 连接使用相同的 Semantix 网络代理设置。

OAuth client 与 token 状态保存在工作区之外、该 server 私有的 Semantix 状态目录中，文件权限
为 `0600`，并绑定完整的 resource URL。显式静态 `Authorization` header 始终优先。
**清除认证** 只删除 Semantix 本地 OAuth 状态，不会退出第三方浏览器会话。Semantix 仅在用户
...
`semantix-agent mcp browse [query]` 与 `semantix-agent mcp install <registry-name>`。Registry
只在用户显式浏览或安装时联网，不进入启动路径。需要 secret 或必填参数的条目只显示为手动配置，
不会写入不完整配置；Registry 故障时可回退到同一查询的缓存结果。

普通配置流程现在只有一步：使用桌面端的“添加并连接”、`/mcp add`，或直接让 Semantix
安装一个 package 或 URL。此类主动安装统一写入用户全局 `config.toml`，安装本身就是授权：
server 会在当前会话连接，现在和下次启动都不会再弹出第二套信任步骤。当前项目
`semantix-agent.toml` 或 `.mcp.json` 中声明的 server 保留在项目配置中，同样默认可信，不需要额外
...
MCP 名称按 workspace 解析：项目声明覆盖同名全局安装；项目内部以 `semantix-agent.toml` 高于
...
[能力诊断](./CAPABILITY_DIAGNOSTICS.zh-CN.md)
（`semantix-agent doctor capabilities` 或 **设置 → 诊断**）。
...
**已有 Claude Code 的 `.mcp.json`？** 直接放到项目根目录，Semantix 会原样读取——其
`mcpServers` 规范（`command`/`args`/`env`、`type`/`url`/`headers`、`${VAR}` 展开）
与 `[[plugins]]` 字段一一对应。两处来源会合并加载；同名时以 `semantix-agent.toml` 为准。
...
**从 `0.x` 升级？** 旧的 `~/.semantix/config.json` 仍会被读取（读其 `mcpServers`、并遵从
`mcpDisabled`），作为最低优先级来源——所以 MCP 服务器照常可用；方便时再把它们挪进
`semantix-agent.toml` 的 `[[plugins]]` 或 `.mcp.json`。
...
是放在 `.semantix/commands/`（项目）或 `~/.semantix/commands/`（用户）下的 Markdown 文件——
...
另一端发现。交互式聊天里使用 `/<name> <任务>` 调用；Semantix 会启动隔离子智能体，
父会话只保留任务和最终答案。

Headless CLI 提供显式管理和运行命令，同时不改变普通 `semantix-agent run` 的任务语义：

```bash
semantix-agent subagent list
semantix-agent subagent create reviewer --description "审查改动" --prompt-file reviewer.md --tools read_file,grep,bash
semantix-agent subagent edit reviewer --effort high --model deepseek-pro
semantix-agent subagent try reviewer "审查当前 diff"   # 始终只读
semantix-agent subagent run reviewer "审查并修复当前 diff"
...
- **常驻指令**来自分层加载的 `SEMANTIX.md`、`AGENTS.md` 和 `CLAUDE.md`。必须在每个
...
每个真实用户回合前，Semantix 会自动召回一小组相关事实。它用原始用户消息搜索，抑制“继续”
...
Semantix 会把 `docs/` 中的 Markdown 文档和已审查的 `release-notes/releases.json` 更新日志
目录随 CLI 和桌面端一起编译发布。只读 `docs` 工具通过本地 BM25 检索这份与当前安装版本
完全一致的离线语料，并可按命中的 `section_id` 读取完整章节及来源。每个版本都会生成
`changelog/v0.5.1.md`、`changelog/v0.5.1.zh-CN.md` 这类中英文虚拟文档，因此可以离线
查询指定版本的新增功能、升级说明、修复和已知风险。涉及 Semantix 配置、CLI/桌面端行为、
...
输入 `/docs <问题>`（例如 `/docs 0.5.1 更新日志`）时，Semantix 会先在本地完成检索，再把
与当前版本匹配的证据交给当前配置的 AI 生成带来源的回答。这个命令路径不依赖模型是否主动
选择 `docs` 工具；普通自然语言问题仍可由模型自动调用该工具。已有自定义命令以及兼容插件或
Skill 别名会继续拥有 `/docs`；发生冲突时，CLI 与桌面端通常会改为通过 `/semantix:docs` 暴露
内置语料。如果这个限定名也已被占用，Semantix 会选择下一个空闲的 `semantix:` 限定后备名，
...
Goal 是长期目标的统一运行机制。Semantix 会持续推进，直到完成、阻塞、暂停或被清除。
...
evaluator 判定。旧 `.semantix/autoresearch/<task-id>/` 目录保持只读：显式引用旧路径时可恢复为
普通 Goal，但新版本不会创建或改写这些目录。旧预算 flags 仅为兼容继续接受，不再出现在帮助和补全中。

## @ 引用

在消息里写 `@` 引用，Semantix 会在发送前解析成带标签的上下文块：`@path/to/file`（或
...
`semantix-agent setup` 现在统一管理 provider、模型列表、凭据、连接测试和默认模型；所有修改
会暂存到“保存并退出”，并同步维护桌面端 provider access。完整用法见
[CLI 命令参考](./CLI.zh-CN.md#配置供应商)。若要让两个模型协同（执行器 + 规划器，
各自独立、缓存稳定的 session），向导后手动在 `semantix-agent.toml` 加一行即可：
...
Planner 会看到已加载的 `SEMANTIX.md` / `AGENTS.md` 记忆，并拿到一小组只读研究工具，
因此可以先检查相关文件再把计划交给执行器。写入类和流程类工具仍只给执行器使用。

Semantix 会用确定性规则路由每一轮，不再调用额外的 classifier 模型：问答、短回复、
...
Semantix 会自动管理正常执行：活跃 Todo 连续 8 个工具调用轮次没有新的完成项、唯一读取、
...
| `semantix-agent review`（CLI） | 只读评审 diff 或分支 |
...
`semantix-agent run --profile delivery "修复并验证这个 bug"`。Economy（轻量）初始只带 9 个工具：
...
`Shift+Tab` 切换到 Plan。Semantix 先生成计划，待用户批准后工作流才切换到实施；规划期间的
...
`semantix-agent config reasoning-language auto|zh|en`。只有明确想为
