...
Semantix exposes a single `/effort` knob (and the per-provider `effort` /
...
|----------------|----------|-------------------|------------------|-------|
| Kimi CN/Global `kimi-k3` | `api.moonshot.cn/v1`, `api.moonshot.ai/v1` | `reasoning_effort` | `low`, `high`, `max` | Always thinks; defaults to `max`. Semantix replays the complete assistant message, uses `max_completion_tokens`, and omits K3's fixed sampling fields. |
...
and custom model IDs. While GLM thinking is enabled, Semantix retains and
...
While this protocol is selected, Semantix always exposes K3's fixed
...
`deepseek-anthropic` entries, keep their configured protocol. Semantix emits
...
Semantix preserves those declared values instead of applying the generic
...
Distinguishing "provider ignores the field" from a Semantix bug starts here:
the request shape Semantix emits is fixed per the table, so a mismatch between
the table and observed behaviour is the provider's, not Semantix's.
