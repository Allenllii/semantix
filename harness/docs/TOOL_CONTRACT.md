...
| `semantix_lookup` | true | Search the semantix slice store — the project's past-session knowledge — for similar tasks or code snippets, and return the top matches as JSON. Read-only; degrades to an empty result when the semantix kernel binary is unavailable. |
