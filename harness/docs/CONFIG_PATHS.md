...
Starting with **Semantix v1.8.1**, Semantix uses one user-facing home directory
for global configuration and user-owned state. CLI and desktop share this
location.

## Semantix Home

| Platform | Semantix home |
| --- | --- |
| macOS | `~/.semantix` |
| Linux | `~/.semantix` |
| Windows | `%APPDATA%\semantix` |

Set `SEMANTIX_HOME` to override Semantix home for tests, CI, or portable
installations. Normal users should not need it.

When `SEMANTIX_HOME` is set, the runtime is fully self-contained: all
...
Advanced test and portable setups may set `SEMANTIX_STATE_HOME` to move runtime
state such as sessions, archives, and memory. It does not move global config or
provider credentials: those remain under `SEMANTIX_HOME`. If an older build wrote
provider keys to `SEMANTIX_STATE_HOME/.env`, Semantix imports those keys
non-destructively when `<Semantix home>/.env` is missing them.

## What Lives There

| Data | Path |
| --- | --- |
| Global config | `<Semantix home>/config.toml` |
| Global provider credentials | `<Semantix home>/.env` |
| Legacy credentials import source | `<Semantix home>/credentials` |
| Global slash commands | `<Semantix home>/commands/` |
| Global skills | `<Semantix home>/skills/` |
| Global hooks | `<Semantix home>/settings.json` |
| Remote-SSH managed known_hosts | `<Semantix home>/remote/known_hosts` |
| Sessions | `<state root>/sessions/` |
| Archives | `<state root>/archive/` |
| Memory | `<state root>/memory/` and `<state root>/projects/` |

`<state root>` defaults to `<Semantix home>`. It only differs when
`SEMANTIX_STATE_HOME` is set.

The global user config is named `config.toml`. Project-local config files keep
the name `semantix-agent.toml`. If someone says "global semantix-agent.toml", they usually
mean `<Semantix home>/config.toml`.

## Global `config.toml`

`<Semantix home>/config.toml` stores non-secret configuration shared by the CLI
and desktop app. It may contain the same provider, plugin, UI, desktop, tool,
skill, sandbox, bot, and agent settings that Semantix renders into user config.
...
also hidden from Semantix's file readers, sandboxed shell commands, and MCP
...
When a custom provider is added from the desktop settings or `semantix-agent setup`,
Semantix stores a generated `api_key_env` in `config.toml` and writes the secret
...
Semantix derives the default from the provider name. Names that normalize to
...
Semantix sends chat requests to `base_url + "/chat/completions"` and probes model
discovery candidates such as `/models` and `/v1/models`. If a gateway gives you a
complete chat request URL, set `chat_url`; Semantix will use it directly and will
...
`<Semantix home>/.env` is the single runtime source for provider API keys saved
by Semantix. The setup wizard, desktop settings, CLI missing-key prompts, and
...
- multiline values are rejected by Semantix writes;
- keys must use shell-style names such as `DEEPSEEK_API_KEY`;
- `# semantix-cleared KEY` comments are non-secret tombstones written after a key
  is deleted so legacy stores do not silently re-import it;
- Semantix writes this file with restricted permissions where the OS supports
  them.

For provider requests, Semantix resolves only this global `.env`. Project `.env`
...
into the process environment, and Semantix control variables such as
`SEMANTIX_HOME`, `SEMANTIX_STATE_HOME`, and `XDG_CONFIG_HOME` are ignored there.

Caches remain in the OS cache directory, for example
`~/Library/Caches/semantix` on macOS, `$XDG_CACHE_HOME/semantix` or
`~/.cache/semantix` on Linux, and `%LOCALAPPDATA%\semantix\cache` on Windows.
Set `SEMANTIX_CACHE_HOME` to override the cache root. When `SEMANTIX_HOME` is
set, the cache is placed under `$SEMANTIX_HOME/cache` (unless
`SEMANTIX_CACHE_HOME` is also set, which takes precedence).
...
> global <Semantix home>/config.toml
...
```text
macOS/Linux: ~/.semantix/config.toml
Windows:     %APPDATA%\semantix\config.toml
```

## Legacy Migration

Starting with **v1.8.1**, Semantix automatically checks legacy locations on
startup before the first config load. Migration is synchronous, one-time, and
non-destructive: old files are copied or converted to Semantix home and left
untouched.

Legacy config sources include:

```text
~/Library/Application Support/semantix/config.toml
~/.config/semantix/config.toml
~/.semantix/semantix-agent.toml
~/.semantix/config.json
```

Legacy credentials, memory files, and sessions are also imported into Semantix
home when the new destination does not already exist. Legacy provider keys are
copied into `<Semantix home>/.env` only when that file does not already contain
...
Starting in **v1.9.1**, Semantix also backfills MCP servers from known legacy
paths, legacy `config.json`, desktop-registered projects, and restored tab
projects into the global `<Semantix home>/config.toml`. Existing global
...
If Semantix has already created the new home directory but some legacy data was
...
```text
/migrate --from "D:\OldSemantix"
```

The explicit form imports sessions only. The path may be the old install
directory, a `.semantix`/data directory, or the `sessions` directory itself;
Semantix checks the common layouts below that root and uses a source-specific
...
existing `<Semantix home>/config.toml`; if the new config already exists, copy
...
- `/migrate` is available only in Go-based Semantix builds that include the
  command. If Semantix reports `unknown command`, upgrade first and rerun it.
