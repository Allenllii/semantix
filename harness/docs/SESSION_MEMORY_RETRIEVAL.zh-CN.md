...
Context Engine v2 为 Semantix 提供两个权限不同的持久上下文层：
...
| 放在哪里 | 适合存放 | 示例 |
| --- | --- | --- |
| `AGENTS.md`、`SEMANTIX.md` 或 `CLAUDE.md` | 每个相关回合都必须存在的规则 | 必跑测试、仓库边界、评审约定 |
...
Semantix 识别 `SEMANTIX.md`、`AGENTS.md`、`CLAUDE.md`，以及对应的 `.local.md`
变体。它先加载 Semantix home 下的用户全局指令，再从 workspace root 逐级走到目标路径；
...
执行，超限会提示把"永远必须遵守的规则"移入 SEMANTIX.md/AGENTS.md instructions。
...
每个真实用户回合开始前，Semantix 会用原始用户消息搜索 active facts。宿主追加给 provider
...
普通路径零配置。只有同时满足以下条件时，Semantix 才可以自动创建一条新记忆：
...
能力时，Semantix 不会回退读取桌面机器的本地 session 或 memory。
