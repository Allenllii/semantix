...
Inside `semantix-agent`, use the slash command:
...
`semantix-agent.toml`; use the shell command with `--local` for that.

Headless runs also use the same setting:

```bash
semantix-agent run "explain this module"
```
...
```text
./semantix-agent.toml > user config.toml > built-in defaults
```
...
Semantix adds the same small transient `<reasoning-language>` block for that
...
When set to `zh` or `en`, Semantix always adds a small transient
