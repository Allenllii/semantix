...
Semantix 只暴露一个 `/effort` 开关（以及 provider 级的 `effort` / `thinking`
...
|----------------------------|--------------------------------------------|-----------------------------------------------|-------------------------------|-------|
| Kimi CN/Global `kimi-k3`   | `api.moonshot.cn/v1`、`api.moonshot.ai/v1` | `reasoning_effort`                            | `low`、`high`、`max`          | 始终思考；默认 `max`。Semantix 会回放完整的 assistant 消息、使用 `max_completion_tokens`，并省略 K3 固定的采样字段。 |
...
GLM 思考开启时，Semantix 会按 GLM 交错与保留思考的要求，在后续历史中原样保留
...
启用该协议后，Semantix 固定展示 K3 的 `auto`/`low`/`high`/`max` 档位，协议默认值
...
provider（包括旧的 `deepseek-anthropic` 条目）保留其原协议。Semantix 会发送
...
档位；在这种情况下，Semantix 会保留这些声明的值，而不是套用通用上限。精选的
...
区分“provider 忽略字段”与 Semantix 自身的 bug 从这里入手：Semantix 发出的
请求形态按表格固定，因此表格与实际行为不一致时，问题在提供商而不是 Semantix。
