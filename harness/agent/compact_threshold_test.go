...
func TestCompactTriggerIndependentOfOutputBudget(t *testing.T) {
	a := &Agent{
		...
		contextWindow: 128_000, agentConfig: agentConfig{compactRatio: defaultCompactRatio},
...
func TestRecentTailBudgetClamp(t *testing.T) {
	...
	for _, tc := range cases {
		a := &Agent{contextWindow: tc.window, agentConfig: agentConfig{compactRatio: defaultCompactRatio}}
...
func TestCheckpointCeilingAndExceptionalSavings(t *testing.T) {
	a := &Agent{contextWindow: 1_000_000, agentConfig: agentConfig{compactRatio: 0.85}}
