...
var knownDirectChildRunners = map[string]string{
	"harness/agent/task.go":    "defines the runners and is the unified path itself",
	"harness/boot/boot.go":     "skill try + run_skill runners",
	"harness/cli/review.go":    "semantix-agent review",
	"desktop/subagents_app.go": "desktop profile preview (not vendored yet)",
}
