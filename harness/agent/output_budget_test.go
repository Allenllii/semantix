...
func TestSharedWindowFoldUsesGuardedInputBudget(t *testing.T) {
	...
	a := &Agent{contextWindow: 100_000, svc: agentServices{prov: prov, sink: event.Discard}, sess: sessionRuntime{output: outputBudgetState{outputBudget: prov.budget}}}
...
func TestEffectiveOutputBudgetClipsSharedWindowRequest(t *testing.T) {
	prov := &sharedWindowTestProvider{budget: 128 * 1024, shared: true}
	a := &Agent{contextWindow: 1_048_576, svc: agentServices{prov: prov}, sess: sessionRuntime{output: outputBudgetState{outputBudget: prov.budget}}}
...
func TestCalibratedOutputBudgetIncludesReplayedReasoning(t *testing.T) {
	prov := &sharedWindowTestProvider{budget: 128 * 1024, shared: true}
	a := &Agent{contextWindow: 200_000, svc: agentServices{prov: prov}, sess: sessionRuntime{output: outputBudgetState{outputBudget: prov.budget}}}
...
func TestPrepareSamplingRequestClipsSharedWindowOutput(t *testing.T) {
	...
	a := &Agent{contextWindow: 1_048_576, agentConfig: agentConfig{compactRatio: 2}, svc: agentServices{prov: prov, tools: tool.NewRegistry()},
...
func TestSummarizeRejectsLengthTruncation(t *testing.T) {
	...
	a := &Agent{contextWindow: 1_048_576, svc: agentServices{prov: prov, sink: event.Discard}, sess: sessionRuntime{output: outputBudgetState{outputBudget: prov.budget}}}
