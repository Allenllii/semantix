...
func TestCompactionAblationCollapsesTheCachePreservingDeferral(t *testing.T) {
	full := &Agent{contextWindow: 100_000, agentConfig: agentConfig{compactRatio: 0.8}}
	if got := full.compactTrigger(); got != 80_000 {
		t.Fatalf("control trigger = %d, want 80000", got)
	}

	off := &Agent{contextWindow: 100_000, agentConfig: agentConfig{compactRatio: 0.8}, ablation: ablation.New(ablation.Compaction)}
