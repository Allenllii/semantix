...
func TestResolveRuntimeCommandExpandsPluginRoot(t *testing.T) {
	root := t.TempDir()
	got, err := resolveRuntimeCommand(&pluginpkg.RuntimeSpec{Command: "${SEMANTIX_PLUGIN_ROOT}/bin/sidecar"}, root)
	if err != nil {
...
func TestRuntimeEnvFullTrustContract(t *testing.T) {
	t.Setenv("SEMANTIX_TEST_INHERITED_MARKER", "present")
	root := t.TempDir()
	...
	if values["SEMANTIX_TEST_INHERITED_MARKER"] != "present" {
