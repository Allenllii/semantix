...
func TestProcessEnvUnfilteredByDefault(t *testing.T) {
	...
	if !strings.Contains(joined, "SEMANTIX_TEST_SECRET_TOKEN=ghp_abcdefghijklmnopqrstuvwxyz") {
	...
	joined = strings.Join(ProcessEnv(), "\n")
	if strings.Contains(joined, "SEMANTIX_TEST_SECRET_TOKEN") {
...
func TestProcessEnvAlwaysFiltersRegisteredCredentialKeys(t *testing.T) {
	const key = "SEMANTIX_TEST_CUSTOM_PROVIDER_CREDENTIAL"
	t.Setenv(key, "opaque-provider-value")
	t.Setenv("SEMANTIX_TEST_BENIGN_ENV", "visible")
	...
	if !strings.Contains(joined, "SEMANTIX_TEST_BENIGN_ENV=visible") {
