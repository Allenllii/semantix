...
func prepareMCPPrivateStateForOS(s Spec, processSandbox sandbox.Spec, env []string, goos string) (sandbox.Spec, []string, error) {
	...
	// short temporary directory. Nesting TEMP below Semantix's workspace-scoped
...
// terminal, even when Semantix was launched from the Finder / Dock / open(1).
...
func defaultStdioShellPATH(ctx context.Context) string {
	...
	const marker = "__SEMANTIX_PATH__="
	script := "printf '\\n" + marker + "%s\\n' \"$PATH\""
