...
// Run with:
//
//	SEMANTIX_LIVE_CHROME_MCP=1 go test ./internal/plugin \
//	  -run '^TestChromeDevtoolsMCPLive$' -v -count=1 -timeout=3m
func TestChromeDevtoolsMCPLive(t *testing.T) {
	if os.Getenv("SEMANTIX_LIVE_CHROME_MCP") != "1" {
		t.Skip("set SEMANTIX_LIVE_CHROME_MCP=1 to run the real Chrome MCP smoke test")
	}
	...
	pageHTML := `data:text/html,<title>Semantix%20MCP</title><h1>Semantix%20MCP%20Ready</h1>`
	...
	out = executeLiveChromeTool(t, callCtx, tools, "wait_for", map[string]any{"text": []string{"Semantix MCP Ready"}, "timeout": 10_000})
	if !strings.Contains(out, "Semantix MCP Ready") {
	...
	out = executeLiveChromeTool(t, callCtx, tools, "evaluate_script", map[string]any{
		"function": `() => { console.log("semantix-mcp-console"); return document.title; }`,
	})
	if !strings.Contains(out, "Semantix MCP") {
	...
	if !strings.Contains(consoleOut, "semantix-mcp-console") || strings.TrimSpace(networkOut) == "" {
