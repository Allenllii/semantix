// Package plugin is Semantix's MCP client. It connects to external MCP servers and
...
// protocolVersion is the MCP revision Semantix advertises during initialize.
...
type Spec struct {
	...
	// for this server. Zero keeps Semantix's built-in default.
	...
	// Semantix's built-in defaultCallTimeout.
	...
	// inherit semantix's cwd (the default for user-configured plugins). It exists
	...
	// stdio servers keep inheriting Semantix's cwd while still receiving the
...
// capabilities Semantix advertises (currently ping and roots/list).
...
type StartPolicy struct {
	...
	// write MCP stats or schema cache files under Semantix home.
...
func (c *Client) withProgress(ctx context.Context, method string, params any) (any, func()) {
	...
	token := fmt.Sprintf("semantix-%d", c.progressID.Add(1))
...
func (c *Client) initializeSession(ctx context.Context, recordCapabilities bool) error {
	...
	res, err := c.call(ctx, "initialize", map[string]any{
		...
		"clientInfo":      map[string]any{"name": "semantix", "version": "dev"},
	})
	if err != nil {
		return err
	}
...
// toolName builds Semantix's canonical model-visible name
...
func (t *remoteTool) ExecuteWithImages(ctx context.Context, args json.RawMessage) (string, []string, error) {
	...
	if tool.HasNonDestructiveMCPExecutionIntent(ctx) {
		...
		if !t.MCPServerAuthorized() || destructive {
			return "", nil, fmt.Errorf("MCP server %q changed the authorization or destructive classification for tool %q; the call was blocked before dispatch — retry so Semantix can re-apply the current Planner MCP safety boundary", t.client.name, t.rawName)
		}
	}
