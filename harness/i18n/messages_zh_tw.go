...
var ChineseTraditional = Messages{
	...
	InitHint: "專案記憶（AGENTS.md）在會話內由模型生成：執行 `semantix-agent`，然後 `/init` —— 模型會分析程式碼庫並寫入。設定請用 `semantix-agent setup`。",
	...
	NoSessionToResume: "沒有可恢復的會話 — 用 `semantix-agent` 開一個新的",
	...
	ApprovalNoteHint:                       "說明要改什麼，Enter 提交，Esc 返回",
	ChatStatusApprovalNote:                 "Enter 提交說明 · Esc 返回選項 · 留空即繼續規劃",
	...
	ArgEffortMax:        "最高推理",
	ArgEffortNone:       "無推理",
	ArgEffortDisabled:   "推理已關閉",
	ArgEffortEnabled:    "推理已開啟",
	ArgEffortAdaptive:   "依請求自適應推理",
	ArgEffortGeneric:    "提供方回報的 effort 檔位",
	ArgThemeCurrent:     "當前",
	...
	WorkModeListHint:             "使用 /work light|balanced|delivery 切換（/preset、/work-mode、/profile 為相容別名）",
	...
	WorkModeUsage:                "用法：/work light|balanced|delivery",
	...
	WorkModeDeprecatedNotice:     "提示：/work-mode 與 /profile 已棄用，請使用 /work light|balanced|delivery",
	RewindNone:                   "暫無可回滾的內容",
	...
	TryHintFmt:               "試試: %s",
	NextHint:                 "下一步：設定 API key（執行 `semantix-agent setup` 或 export DEEPSEEK_API_KEY=...），然後執行 `semantix-agent run \"你的任務\"`。",
	...
	SetupAddPreset:           "從推薦預設新增",
	SetupAddPresetDesc:       "GLM、Kimi、Qwen、MiniMax… 一鍵範本",
	SetupAddPresetLabel:      "推薦供應商 · 選一個新增",
	...
	RemoteNoHostsHint:         "尚未設定遠端主機；用 `semantix-agent remote add <名稱> [user@]host` 新增",
	...
	ProviderErrAuth:                "認證失敗 (HTTP 401)：未讀到 API key（缺失或未設定）。請在 .env 中設定金鑰，或執行 `semantix-agent setup`。",
	...
	ReportUsageBody: `用法：
  semantix-agent report                 預覽最新本機報告並確認是否傳送
  semantix-agent report list            列出本機 CLI 當機報告
  semantix-agent report show [ID]       預覽報告（省略 ID 時使用最新一份）
  ...
  semantix-agent report delete [ID]     不傳送，直接刪除本機報告`,
	...
	UsageBody: `semantix-agent — 由設定和插件驅動的 coding agent（多模型）

用法：
  semantix-agent [--model NAME] [-c|--continue] [-r|--resume [QUERY]] [--permission-mode MODE] [--effort LEVEL] [--add-dir PATH]   互動式會話
  ...
  semantix-agent run --events-jsonl [--model NAME] <task>      輸出脫敏結構化事件 JSONL
  semantix-agent review [--base BRANCH] [--commit SHA] [--model NAME]  AI 程式碼審查（基於本機 diff）
  ...
  semantix-agent doctor session <branch-id> [--zip] [--out PATH]  匯出會話衝突診斷 zip
  semantix-agent session list --json [--dir PATH]             為機器客戶列出脫敏會話
  semantix-agent session show|status <machine-session-id> --json [--dir PATH]  查詢單一脫敏會話
  semantix-agent session recovery [<machine-session-id>] --json [--dir PATH]  查詢脫敏復原狀態
  semantix-agent hook list|status --json [--dir PATH]         檢視脫敏 Hook 狀態
  semantix-agent task list|show|status|events|stop|cancel|monitor|tmux --json [--dir PATH]
                                                         檢視或控制脫敏 Task
  semantix-agent bot start|doctor|weixin-login                多管道 IM bot 閘道
  semantix-agent upgrade [--check] [--force]                   更新到最新正式版（別名：semantix-agent update）
  semantix-agent completion bash|zsh|fish                     列印 shell 補全腳本到 stdout
  semantix-agent version [--verbose|--json]                   列印版本（單行）或建置元資訊
  semantix-agent --version | -v                               單行版本（腳本安全）
  semantix-agent help
...
範例：
  ...
  semantix-agent web
  semantix-agent run "把 main.go 裡的 TODO 實現掉"
  semantix-agent run --model mimo-pro "給這個函式補單元測試"
  semantix-agent -p "總結這個倉庫" --output-format json
  semantix-agent subagent run review "審查目前變更"
...
`,
	...
	EffortNotConfigurableFmt:    "%s 不支援設定 effort",
	EffortCurrentFmt:            "%s 的 effort：%s（預設：%s；選項：%s）",
	EffortListHeaderFmt:         "%s 的 reasoning effort",
	EffortListHint:              "用 /effort <檔位> 切換（auto 表示顯式採用已設定的預設值）",
	EffortUsageFmt:              "用法：/effort %s",
	EffortNoConfigDir:           "effort：無法解析使用者設定目錄",
	EffortSwitchUnavailable:     "本會話不支援切換 effort",
	EffortSwitchBusy:            "請先完成或取消目前工作並停止背景任務，再變更 effort",
	EffortSwitchPending:         "請等待目前執行階段切換完成",
	EffortSwitchedFmt:           "%s 的 effort 已設為 %s（後續回合生效，不重建會話）",
	...
	ProviderErrAuthRejected:    "認證失敗 (HTTP 401)：服務端拒絕了你的 API key。可能是 key 錯誤或已過期，也可能是服務端出現瞬時鑑權/額度問題——已退避重試仍失敗。請稍後再試，或檢查 .env 中的金鑰 / 執行 `semantix-agent setup`。",
