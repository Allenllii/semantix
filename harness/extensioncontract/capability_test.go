...
func TestCapabilityKeyValidate(t *testing.T) {
	if err := (CapabilityKey{Namespace: "semantix", Kind: "provider", ID: "deepseek/v4"}).Validate(); err != nil {
		t.Fatal(err)
	}
...
func TestRequirementSatisfiedBy(t *testing.T) {
	provided := Capability{
		Key:        CapabilityKey{Namespace: "semantix", Kind: "provider", ID: "deepseek/v4"},
		Version:    "1.2.0",
		SchemaHash: "sha256:p",
	}
	req := Requirement{
