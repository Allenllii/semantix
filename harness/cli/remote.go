...
import (
	...
	"semantix/harness/remote"
)

// remoteCommand dispatches `semantix-agent remote <sub>`, mirroring mcpCommand.
...
func removedWorkbenchCommand(name string) int {
	fmt.Fprintf(os.Stderr, "semantix-agent remote %s: Remote Workbench 已移除，请使用 `semantix-agent remote connect <host> --open`\n", name)
	return 1
}
...
// LoadForRoot), so they must never be written to a project semantix-agent.toml.
...
const remoteAddUsage = "usage: semantix-agent remote add <name> [user@]host[:port] [flags]"
...
func remoteRemoveCLI(args []string) int {
	if len(args) != 1 {
		fmt.Fprintln(os.Stderr, "usage: semantix-agent remote remove <name>")
		return 2
	}
	name := args[0]
	removed := false
...
func remoteTestCLI(args []string) int {
	if len(args) != 1 {
		fmt.Fprintln(os.Stderr, "usage: semantix-agent remote test <name|user@host>")
		return 2
	}
...
Usage:
  semantix-agent remote add <name> [user@]host[:port] [--identity F] [--jump SPEC]
                     [--workspace PATH] [--use-ssh-config] [--serve-install auto|npm|upload|never]
                     [--passphrase-env NAME] [--password-env NAME]
  semantix-agent remote list
  semantix-agent remote remove <name>
  semantix-agent remote import [alias...|--all]      # from ~/.ssh/config
  semantix-agent remote test <name|user@host>        # dial + auth + host-key check
  semantix-agent remote connect <name> [--workspace PATH] [--local-port N] [--no-serve] [--open] [--forward-only]
  semantix-agent remote open <name>                  # connect --open
  semantix-agent remote status [<name>]
  semantix-agent remote forward add <host> (-L|-R) <spec> | forward rm <host> <name> | forward ls <host>
  semantix-agent remote serve start|stop|status|logs <name> [--workspace PATH] [-n N]
  semantix-agent remote fs ls <name>:<path> | fs get <name>:<remote> [local] | fs put <local> <name>:<remote>`)
}
