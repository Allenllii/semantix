...
func sessionsCommand(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: semantix-agent sessions <reindex|diagnose|cleanup> [--dir PATH] [--json]")
		return 2
	}
	switch args[0] {
	case "diagnose":
	...
	if fs.NArg() != 0 {
		fmt.Fprintln(os.Stderr, "usage: semantix-agent sessions reindex [--dir PATH] [--json]")
		return 2
	}
	if len(dirs) == 0 {
...
func sessionsRecoveryCommand(args []string, cleanup bool) int {
	...
	if fs.NArg() != 0 || (!cleanup && *apply) {
		fmt.Fprintf(os.Stderr, "usage: semantix-agent %s [--dir PATH] [--json]", name)
		if cleanup {
