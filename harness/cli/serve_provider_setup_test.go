...
const serveMissingKeyHelperEnv = "SEMANTIX_TEST_SERVE_MISSING_KEY_HELPER"
...
func TestServeStartsWithMissingProviderKey(t *testing.T) {
	if os.Getenv(serveMissingKeyHelperEnv) == "1" {
		...
		if err := os.Setenv("SEMANTIX_HOME", os.Getenv("SEMANTIX_TEST_SERVE_HOME")); err != nil {
			os.Exit(2)
		}
		code := runServe([]string{
			"--model", "remote-demo/model-a",
			"--addr", "127.0.0.1:0",
			"--port-file", os.Getenv("SEMANTIX_TEST_SERVE_PORT_FILE"),
			"--auth", "token",
...
models = ["model-a"]
default = "model-a"
api_key_env = "SEMANTIX_TEST_REMOTE_MISSING_KEY"
`
	...
	cmd.Env = replaceServeTestEnv(os.Environ(),
		...
		"SEMANTIX_HOME="+home,
		"SEMANTIX_TEST_SERVE_HOME="+home,
		"SEMANTIX_CREDENTIALS_STORE=file",
		"SEMANTIX_TEST_REMOTE_MISSING_KEY=",
		"SEMANTIX_TEST_SERVE_PORT_FILE="+portFile,
	)
	...
	if !bytes.Contains(body, []byte("Semantix Provider Setup")) {
