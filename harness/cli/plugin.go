...
import (
	...
	"semantix/harness/installsource"
...
func pluginUsage() {
  ...
  semantix-agent plugin install <source> [--yes] [--dry-run] [--link] [--replace]
  semantix-agent plugin list
  semantix-agent plugin show <name>
  semantix-agent plugin enable <name>
  semantix-agent plugin disable <name>
  semantix-agent plugin remove <name>
  semantix-agent plugin doctor <name>
  semantix-agent plugin migrate <name> --to-v2`)
}
...
func pluginListCommand() int {
	st, err := pluginpkg.LoadState(config.SemantixHomeDir())
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		return 1
	}
...
func pluginShowCommand(args []string) int {
	...
	root := pluginpkg.ResolveRoot(config.SemantixHomeDir(), p.Root)
...
func pluginDoctorCommand(args []string) int {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "missing apiVersion") {
			fmt.Fprintf(os.Stderr, "remediation: semantix-agent plugin migrate %s --to-v2\n", args[0])
		}
		return 1
	}
...
// something runnable. ${SEMANTIX_PLUGIN_ROOT} expands to the installed root;
...
func pluginSetEnabledCommand(args []string, enabled bool) int {
	...
	if err := pluginpkg.SetEnabled(config.SemantixHomeDir(), args[0], enabled); err != nil {
		fmt.Fprintln(os.Stderr, err)
		return 1
	}
