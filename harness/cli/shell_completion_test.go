...
func TestCLICompletionListsRootAndNestedCommands(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 1, []string{"semantix", "co"}, values)
	if want := []string{"config", "completion"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("root completion = %v, want %v", got, want)
	}

	got = cliCompletionCandidatesWithValues(root, 2, []string{"semantix", "mcp", "b"}, values)
	if want := []string{"browse"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("mcp subcommand completion = %v, want %v", got, want)
	}

	got = cliCompletionCandidatesWithValues(root, 3, []string{"semantix", "remote", "serve", "st"}, values)
...
func TestCLICompletionListsCommandFlags(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 2, []string{"semantix", "run", "--m"}, values)
	...
	got = cliCompletionCandidatesWithValues(root, 1, []string{"semantix", "--d"}, values)
	...
	got = cliCompletionCandidatesWithValues(root, 3, []string{"semantix", "mcp", "add", "--h"}, values)
...
func TestCLICompletionUsesConfiguredModelsAndSessionIDs(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 3, []string{"semantix", "run", "--model", "deep"}, values)
	if want := []string{"deepseek/deepseek-chat"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("model completion = %v, want %v", got, want)
	}

	got = cliCompletionCandidatesWithValues(root, 1, []string{"semantix", "--model=mi"}, values)
	if want := []string{"--model=mimo/mimo-v2"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("inline model completion = %v, want %v", got, want)
	}

	got = cliCompletionCandidatesWithValues(root, 2, []string{"semantix", "--resume", "b"}, values)
...
func TestCLICompletionTaskNestedAndRunAblate(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 2, []string{"semantix", "task", "st"}, values)
	for _, want := range []string{"status", "stop"} {
		if !containsCompletionValue(got, want) {
			t.Fatalf("task prefix st missing %q: %v", want, got)
		}
	}
	got = cliCompletionCandidatesWithValues(root, 2, []string{"semantix", "task", "mon"}, values)
	if !containsCompletionValue(got, "monitor") {
		t.Fatalf("task monitor missing: %v", got)
	}
	got = cliCompletionCandidatesWithValues(root, 3, []string{"semantix", "task", "monitor", "st"}, values)
	if !containsCompletionValue(got, "status") || !containsCompletionValue(got, "stop") {
		t.Fatalf("task monitor st = %v", got)
	}
	got = cliCompletionCandidatesWithValues(root, 2, []string{"semantix", "run", "--a"}, values)
...
func TestCLICompletionOptionalResumeThenFlag(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 2, []string{"semantix", "--resume", "--m"}, values)
	...
	got = cliCompletionCandidatesWithValues(root, 1, []string{"semantix", "--resume=a"}, values)
...
func TestCLICompletionRunServeResumeRequiresValue(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 3, []string{"semantix", "run", "--resume", "--m"}, values)
	if containsCompletionValue(got, "--model") {
	...
	got = cliCompletionCandidatesWithValues(root, 3, []string{"semantix", "run", "--resume", "b"}, values)
	if want := []string{"beta-session"}; !reflect.DeepEqual(got, want) {
	...
	got = cliCompletionCandidatesWithValues(root, 3, []string{"semantix", "serve", "--resume", "a"}, values)
	if len(got) != 0 {
	...
	got = cliCompletionCandidatesWithValues(root, 2, []string{"semantix", "run", "--resume=b"}, values)
...
func TestCLICompletionTaskPerOperationFlags(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 3, []string{"semantix", "task", "status", "--p"}, values)
	if containsCompletionValue(got, "--project-root") {
		t.Fatalf("task status must not offer --project-root: %v", got)
	}
	got = cliCompletionCandidatesWithValues(root, 3, []string{"semantix", "task", "status", "--j"}, values)
	if !containsCompletionValue(got, "--json") {
		t.Fatalf("task status missing --json: %v", got)
	}

	// events has --jsonl/--after/--follow.
	got = cliCompletionCandidatesWithValues(root, 3, []string{"semantix", "task", "events", "--"}, values)
	...
	got = cliCompletionCandidatesWithValues(root, 3, []string{"semantix", "task", "stop", "--"}, values)
	...
	got = cliCompletionCandidatesWithValues(root, 3, []string{"semantix", "task", "list", "--p"}, values)
	if !containsCompletionValue(got, "--project-root") {
		t.Fatalf("task list missing --project-root: %v", got)
	}

	// tmux attach has --session.
	got = cliCompletionCandidatesWithValues(root, 4, []string{"semantix", "task", "tmux", "attach", "--s"}, values)
...
func TestCLICompletionPathFlagReturnsEmptyForShellFallback(t *testing.T) {
	...
	got := cliCompletionCandidatesWithValues(root, 3, []string{"semantix", "run", "--dir", "do"}, values)
	if len(got) != 0 {
