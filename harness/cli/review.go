...
func reviewCommand(args []string) int {
	...
	// every `semantix-agent review` invocation.
	cfg, err := config.Load()
	if err != nil {
...
func buildReviewSubagentRegistry(reviewSk skill.Skill, cfg *config.Config, root string) *tool.Registry {
	...
	// [sandbox] config, so `semantix-agent review` previously read forbid_read
	...
	if reviewSk.ReadOnly {
		...
		// permission-classified read-only policy) so `semantix-agent review` is not a
		// writable backdoor.
