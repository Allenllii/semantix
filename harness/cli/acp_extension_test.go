...
const (
	acpFakeEnvEnable     = "SEMANTIX_ACP_FAKE_SIDECAR"
	acpFakeEnvPluginName = "SEMANTIX_ACP_FAKE_PLUGIN_NAME"
)
...
func writeACPFixture(t *testing.T, dir string) {
	t.Helper()
	if err := os.WriteFile(filepath.Join(dir, "semantix-agent.toml"), []byte(`
...
func TestACPSessionWithPluginModelStreamsAndSwitches(t *testing.T) {
	...
	installACPFakeProviderPlugin(t, config.SemantixHomeDir(), name)
