...
func TestParseCommandFlagsSuccessProceedsSilently(t *testing.T) {
	...
	var proceed bool
	stderr := captureStderr(t, func() {
		code, proceed = parseCommandFlags(fs, []string{"--name", "semantix"})
	})
	if code != 0 || !proceed || *name != "semantix" {
		t.Fatalf("parseCommandFlags success = (%d, %v, %q), want (0, true, semantix)", code, proceed, *name)
	}
	if stderr != "" {
