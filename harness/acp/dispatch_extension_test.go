...
import (
	...
	"semantix/harness/extension/uihub"
)
...
func TestUpdateSinkExtensionSupportedClientGetsMetaAndText(t *testing.T) {
	...
	vendor, _ := meta["semantix.io"].(map[string]any)
	surface, _ := vendor["extensionSurface"].(map[string]any)
	if surface == nil {
		t.Fatalf("structured update missing _meta.semantix.io.extensionSurface: %v", u)
	}
...
func TestInitializeAdvertisesExtensionSurface(t *testing.T) {
	...
	vendor, ok := ir.AgentCapabilities.Meta["semantix.io"].(SemantixExtensionCapabilities)
	if !ok {
		t.Fatalf("_meta[semantix.io] = %T", ir.AgentCapabilities.Meta["semantix.io"])
	}
	if vendor.ExtensionSurface == nil || !vendor.ExtensionSurface.Supported ||
		vendor.ExtensionSurface.SchemaVersion != semantixExtensionSurfaceSchemaVersion {
	...
	got := decoded.AgentCapabilities.Meta["semantix.io"].ExtensionSurface
	if got == nil || !got.Supported || got.SchemaVersion != semantixExtensionSurfaceSchemaVersion {
...
func TestClientExtensionSurfaceSupportedParsing(t *testing.T) {
	tests := []struct {
		...
		{"capability absent", map[string]any{"semantix.io": map[string]any{}}, false},
		{"supported", map[string]any{"semantix.io": map[string]any{
			"extensionSurface": map[string]any{"supported": true, "schemaVersion": 1},
		}}, true},
		{"explicit false", map[string]any{"semantix.io": map[string]any{
			"extensionSurface": map[string]any{"supported": false},
		}}, false},
		{"malformed vendor", map[string]any{"semantix.io": "nope"}, false},
		{"malformed capability", map[string]any{"semantix.io": map[string]any{
			"extensionSurface": "nope",
		}}, false},
		{"malformed flag", map[string]any{"semantix.io": map[string]any{
