...
func invalidACPv1PermissionOptionKind(options []PermissionOption) (PermissionOption, bool) {
	...
	// own cross-session persistence, so Semantix-specific persistent approvals must
...
func TestUpdateSinkPermissionCarriesStructuredContext(t *testing.T) {
	fn := &fakeNotifier{onReq: func(_ string, params any) (json.RawMessage, error) {
		...
		meta, ok := p.ToolCall.Meta["semantix.io"].(map[string]any)
...
func TestPermissionMetaOnlyTrustsForegroundStaticBash(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "tilde expansion", rawInput: `{"command":"test -f ~/.config/semantix-agent.toml"}`},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			semantix, ok := meta["semantix.io"].(map[string]any)
			if !ok {
				t.Fatalf("semantix-agent metadata = %#v", meta)
			}
			argv, present := semantix["argv"]
			...
			if semantix["commandSchemaVersion"] != 1 || semantix["cwd"] != filepath.Clean(cwd) {
				t.Fatalf("trusted command metadata = %#v", semantix)
			}
		})
	}
}
