// Package releaseasset downloads and verifies immutable Semantix CLI release
// artifacts for a requested platform. It is used when a local Desktop or CLI
// needs to provision the remote `semantix-agent serve` binary without requiring
...
const (
	cliReleaseBase       = "https://github.com/Gnosil/semantix/releases/download"
...
func downloadCLIFromBase(ctx context.Context, client *http.Client, base, version, goos, goarch string, official bool) ([]byte, error) {
	...
	assetName := fmt.Sprintf("semantix-agent-%s-%s.tar.gz", goos, goarch)
...
func fetchBounded(ctx context.Context, client *http.Client, rawURL string, limit int64) ([]byte, error) {
	...
	req.Header.Set("User-Agent", "semantix-remote-bootstrap")
...
func extractCLI(archive []byte) ([]byte, error) {
	...
	for {
		...
		if path.Base(path.Clean(header.Name)) != "semantix-agent" {
			continue
		}
		if header.Typeflag != tar.TypeReg || header.Size <= 0 || header.Size > maxExtractedCLIBytes {
			return nil, errors.New("semantix archive entry is not a bounded regular file")
		}
		if binary != nil {
			return nil, errors.New("semantix archive contains duplicate binaries")
		}
		...
		if int64(len(binary)) != header.Size {
			return nil, errors.New("semantix archive entry size mismatch")
		}
	}
	if len(binary) == 0 {
		return nil, errors.New("semantix binary not found in archive")
	}
	return binary, nil
}
