// Package extension is the Go SDK for Semantix extension sidecars speaking
// Extension Protocol v2 over stdio. An extension is a separate process: the
// Semantix host launches it, sends extension/initialize first, drives
...
// internal/extension/protocol/schema.generated.json in the Semantix
// repository.
package extension
...
func Serve(ctx context.Context, h Handler, opts Options) error {
	...
	logger := opts.Logger
	if logger == nil {
		logger = log.New(os.Stderr, "semantix-extension: ", log.LstdFlags)
	}
	s := &server{
		handler: h,
		opts:    opts,
