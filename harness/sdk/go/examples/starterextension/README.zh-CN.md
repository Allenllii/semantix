...
plugin_root="$(pwd -P)"
semantix-agent plugin install "$plugin_root" --dry-run
semantix-agent plugin install "$plugin_root" --link --replace --yes
```
...
semantix-agent plugin install $pluginRoot --dry-run
semantix-agent plugin install $pluginRoot --link --replace --yes
```

安装前请检查 dry-run 输出中的 `FULL TRUST` 区块。链接安装会持续信任本目录
后续的内容变化，而且代码型 Runtime 运行在 Semantix Sandbox 之外。
...
`semantix-agent plugin doctor starter-extension`。
