...
func TestBashMergesLoginShellPath(t *testing.T) {
	...
	probe := filepath.Join(bin, "semantix-path-probe")
	...
	args, _ := json.Marshal(map[string]string{"command": "semantix-path-probe"})
...
func TestBashCommandEnvFiltersSensitiveKeysWhenEnabled(t *testing.T) {
	...
	t.Setenv("SEMANTIX_TEST_VISIBLE", "ok")
	...
	if !strings.Contains(env, "SEMANTIX_TEST_VISIBLE=ok") {
...
func TestParseShellPATH(t *testing.T) {
	const marker = "__SEMANTIX_BASH_PATH__="
	cases := []struct {
		name string
		out  string
		want string
...
func TestRunShellPATHCommandFiltersEnvWhenEnabled(t *testing.T) {
	...
	secrets.SetFilterSubprocessEnv(true)
	t.Cleanup(func() { secrets.SetFilterSubprocessEnv(false) })
	t.Setenv("SEMANTIX_TEST_SECRET_TOKEN", "ghp_abcdefghijklmnopqrstuvwxyz")

	out := runShellPATHCommand(context.Background(), "/bin/sh", []string{"-c", `printf 'tok=%s' "${SEMANTIX_TEST_SECRET_TOKEN:-none}"`})
