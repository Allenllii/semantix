...
// stateRootFor builds a fake Semantix state root with the two guarded session
...
func TestSessionDataGuardDeniesSessionStores(t *testing.T) {
	...
	for _, target := range []string{
		...
		} else if !strings.Contains(err.Error(), "Semantix's own session/state data") {
...
func TestSessionDataGuardZeroValueUnconfined(t *testing.T) {
	...
	if hint := g.CommandHint("", "rm -rf ~/.semantix/sessions"); hint != "" {
...
func TestSessionDataGuardCommandHintEnvVarForm(t *testing.T) {
	...
	state := filepath.Join(home, ".semantix")
	...
	for _, cmd := range []string{
		`python3 -c "open('$HOME/.semantix/sessions/x.jsonl','w')"`,
		"rm ${HOME}/.semantix/projects/slug/sessions/y.jsonl",
	} {
		if hint := g.CommandHint("", cmd); hint == "" {
...
func TestBashAppendsSessionDataHint(t *testing.T) {
	...
	if !strings.Contains(out, "WARNING: this command referenced Semantix's own session/state data") {
