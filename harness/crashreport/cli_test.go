...
func TestCapturePanicWritesBoundedSanitizedReport(t *testing.T) {
	...
	stack := "goroutine 7 [running]:\n" +
		"semantix/harness/agent.run(" + secret + ")\n" +
	...
	if !strings.Contains(report.Stack, "semantix/harness/agent.run(...)") || !strings.Contains(report.Stack, "<path>/run.go:42") {
		t.Fatalf("sanitized stack = %q", report.Stack)
	}
	if report.TopFrame != "semantix/harness/agent.run <path>/run.go:42" {
