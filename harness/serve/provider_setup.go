...
func (s *Server) refreshProviderSetup(ref string) {
	...
	if lockErr != nil {
		next.Error = "Unable to inspect the remote Semantix credentials."
	} else {
		...
		if err != nil {
			next.Error = "Unable to load the remote Semantix configuration."
...
func (s *Server) configureProviderCredential(ctx context.Context, key string) error {
	...
	if err := s.switchModelLocked(ctx, setup.ModelRef); err != nil {
		...
		s.providerSetup.Error = "The credential was saved, but the Provider could not be activated. Retry or restart Semantix Serve."
		s.providerSetupMu.Unlock()
