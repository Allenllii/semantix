...
func TestNormalizeCommandRepairsOnlyPowerShellFileEscapedQuotes(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			command: `powershell -File \"C:\Users\Example\.semantix\hooks\archive-attachments.ps1\"`,
			want:    `powershell -File "C:\Users\Example\.semantix\hooks\archive-attachments.ps1"`,
		},
		{
			name:    "pwsh file path with spaces",
			command: `pwsh.exe -NoProfile -NonInteractive -File \"C:\Program Files\Semantix Hooks\archive attachments.ps1\"`,
			want:    `pwsh.exe -NoProfile -NonInteractive -File "C:\Program Files\Semantix Hooks\archive attachments.ps1"`,
		},
		{
			name:    "doubly escaped copied quotes",
			command: `pwsh -File \\\"C:\Program Files\Semantix Hooks\archive attachments.ps1\\\" \"arg with spaces\"`,
			want:    `pwsh -File "C:\Program Files\Semantix Hooks\archive attachments.ps1" "arg with spaces"`,
		},
		{
		...
		},
		{
			...
			command: `powershell -NoProfile -File "C:\Program Files\Semantix Hooks\archive attachments.ps1"`,
			want:    `powershell -NoProfile -File "C:\Program Files\Semantix Hooks\archive attachments.ps1"`,
		},
		{
...
func TestLoadNormalizesPowerShellFileEscapedQuotes(t *testing.T) {
	home := t.TempDir()
	bad := `powershell -File \"C:\Program Files\Semantix Hooks\archive attachments.ps1\"`
	want := `powershell -File "C:\Program Files\Semantix Hooks\archive attachments.ps1"`
...
func TestRepairablePowerShellFileArgs(t *testing.T) {
	command := `powershell -NoProfile -NonInteractive -File \"C:\Program Files\Semantix Hooks\archive attachments.ps1\" -Mode \"startup\"`
	...
	wantArgs := []string{"-NoProfile", "-NonInteractive", "-File", `C:\Program Files\Semantix Hooks\archive attachments.ps1`, "-Mode", "startup"}
...
func installSuperpowersV611HookFixture(t *testing.T, home string) string {
	...
	root := filepath.Join(semantixHome, "plugins", "superpowers fixture")
...
func TestLoadSuperpowersV620PreservesExplicitBashRequirement(t *testing.T) {
	home := t.TempDir()
	root := filepath.Join(home, ".semantix", "plugins", "superpowers")
	writeHookTestFile(t, filepath.Join(root, pluginpkg.CodexManifest), `{
  "name": "superpowers",
  "version": "6.2.0",
  "skills": "./skills/"
  ...
  "hooks": {
	...
	if err := pluginpkg.Upsert(filepath.Join(home, ".semantix"), pluginpkg.InstalledPlugin{
		Name:         "superpowers",
...
func TestPluginExplicitBashCommandUsesPOSIXCompatibleRoot(t *testing.T) {
	root := `C:\Users\Test User\AppData\Roaming\semantix\plugins\superpowers`
	config := pluginHookExecutionConfigForPlatform(pluginpkg.Hook{
		Command:      `"${CLAUDE_PLUGIN_ROOT}/hooks/run-hook.cmd" session-start`,
		ShellCommand: true,
		Shell:        "bash",
	}, root, "windows")
	want := `"C:/Users/Test User/AppData/Roaming/semantix/plugins/superpowers/hooks/run-hook.cmd" session-start`
...
func TestLoadIncludesPluginSessionStartHook(t *testing.T) {
  ...
  "skills": "./skills/"
	...
	if got[0].Env["SEMANTIX_PLUGIN_NAME"] != "superpowers" || got[0].Env["SEMANTIX_WORKSPACE_ROOT"] != "/workspace" {
...
// (the hook-machine default after #7420) and an isolated SEMANTIX_HOME, the
// plugin probe and global settings resolve from the platform Semantix home —
// <home>/plugins and <home>/settings.json — not a doubled .semantix segment.
func TestInspectNoHomeDirResolvesPluginRootFromPlatformHome(t *testing.T) {
	...
	// Global settings live directly under the platform Semantix home
	// (writeSettings would add .semantix, the OS-home convention #7420 fixes).
  ...
  "skills": "./skills/"
	...
	// platform Semantix home, not a doubled .semantix segment.
...
func TestLoadIncludesPluginClaudeCompatibilityHooks(t *testing.T) {
	home := t.TempDir()
	semantixHome := filepath.Join(home, ".semantix")
	root := filepath.Join(semantixHome, "plugins", "claude-pack")
	writeHookTestFile(t, filepath.Join(root, pluginpkg.CodexManifest), `{
  "name": "claude-pack",
  "version": "1.0.0",
  "skills": "skills"
  ...
  "hooks": {
	...
	if h := byEvent[PostToolUse]; h.Env["CLAUDE_PROJECT_DIR"] != "/workspace" || h.Env["SEMANTIX_PLUGIN_NAME"] != "claude-pack" {
...
func TestLoadPluginHooksPreservesExecutionContract(t *testing.T) {
	home := t.TempDir()
	semantixHome := filepath.Join(home, ".semantix")
	root := filepath.Join(semantixHome, "plugins", "hook-contract")
	writeHookTestFile(t, filepath.Join(root, pluginpkg.NativeManifest), `{
  "apiVersion": "semantix.io/plugin/v2", "name": "hook-contract",
  "hooks": {
    "SessionStart": [
...
func TestLoadExpandsSemantixPluginRootBeforeShellLaunch(t *testing.T) {
	home := t.TempDir()
	semantixHome := filepath.Join(home, ".semantix")
	root := filepath.Join(semantixHome, "plugins", "impeccable")
	projectRoot := filepath.Join(home, "$CLAUDE_PLUGIN_ROOT-project")
	writeHookTestFile(t, filepath.Join(root, pluginpkg.NativeManifest), `{
  "apiVersion": "semantix.io/plugin/v2", "name": "impeccable",
  "version": "3.9.1",
  "hooks": {
    "PostToolUse": [{
      "match": "edit",
      "command": "node \"${SEMANTIX_PLUGIN_ROOT}/skills/impeccable/scripts/hook.mjs\"",
      "shellCommand": true,
      "cwd": "${SEMANTIX_PLUGIN_ROOT}/work",
      "env": {"IMPECCABLE_CACHE": "%SEMANTIX_PLUGIN_ROOT%/cache"}
    }]
  }
}`)
	if err := pluginpkg.Upsert(semantixHome, pluginpkg.InstalledPlugin{
	...
	if got[0].Env["CLAUDE_PROJECT_DIR"] != projectRoot || got[0].Env["SEMANTIX_WORKSPACE_ROOT"] != projectRoot {
		t.Fatalf("host-provided workspace paths were expanded: %#v", got[0].Env)
	}
}

func TestExpandPluginRootSupportsClaudeSemantixAndCmdAliases(t *testing.T) {
	root := `C:\Program Files\Semantix\plugins\impeccable`
	for _, token := range []string{
		"${CLAUDE_PLUGIN_ROOT}", "$CLAUDE_PLUGIN_ROOT", "%CLAUDE_PLUGIN_ROOT%",
		"${SEMANTIX_PLUGIN_ROOT}", "$SEMANTIX_PLUGIN_ROOT", "%SEMANTIX_PLUGIN_ROOT%",
	} {
	...
	if got, want := expandPluginRoot(`$CLAUDE_PLUGIN_ROOT/$SEMANTIX_PLUGIN_ROOT`, root), root+"/"+root; got != want {
		t.Fatalf("both root aliases = %q, want %q", got, want)
	}
}

func TestExpandPluginRootDoesNotReprocessResolvedRoot(t *testing.T) {
	root := `/tmp/$SEMANTIX_PLUGIN_ROOT/%CLAUDE_PLUGIN_ROOT%/${CLAUDE_PLUGIN_ROOT}`
	value := `${CLAUDE_PLUGIN_ROOT}|$SEMANTIX_PLUGIN_ROOT|%CLAUDE_PLUGIN_ROOT%`
	want := root + "|" + root + "|" + root
...
func TestWindowsBatchCommandLinePreservesQuotedPluginPath(t *testing.T) {
	command := `"C:\Users\Test User\AppData\Roaming\semantix\plugins\superpowers/hooks/run-hook.cmd" session-start`
	got, ok := windowsBatchCommandLine(command)
	if !ok {
		t.Fatal("quoted plugin batch command was not recognized")
	}
	want := `cmd.exe /d /s /c ""C:\Users\Test User\AppData\Roaming\semantix\plugins\superpowers\hooks\run-hook.cmd" session-start"`
	if got != want {
...
func TestWindowsBatchArgvCommandLineSupportsNativePluginHooks(t *testing.T) {
	got, ok := windowsBatchArgvCommandLine(
		`C:\Program Files\Semantix\plugins\example/hooks/run-hook.cmd`,
		[]string{"session-start", "argument with spaces"},
	)
	if !ok {
		t.Fatal("native plugin batch command was not recognized")
	}
	want := `cmd.exe /d /s /c ""C:\Program Files\Semantix\plugins\example\hooks\run-hook.cmd" session-start "argument with spaces""`
	if got != want {
...
func TestSemantixHomeOverridesGlobalHookPaths(t *testing.T) {
	home := t.TempDir()
	semantixHome := filepath.Join(t.TempDir(), "rx-home")
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	t.Setenv("SEMANTIX_HOME", semantixHome)
	if err := os.MkdirAll(semantixHome, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(semantixHome, SettingsFilename), []byte(`{"hooks":{"PostToolUse":[{"command":"echo rx"}]}}`), 0o644); err != nil {
		t.Fatal(err)
	}
	writeSettings(t, home, `{"hooks":{"PostToolUse":[{"command":"echo old"}]}}`)

	if got := GlobalSettingsPath(""); got != filepath.Join(semantixHome, SettingsFilename) {
		t.Fatalf("GlobalSettingsPath = %q, want Semantix home", got)
	}
	hooks := Load(LoadOptions{})
	if len(hooks) != 1 || hooks[0].Command != "echo rx" {
		t.Fatalf("Load hooks = %+v, want Semantix home hook only", hooks)
	}
}

func TestLoadOptionsSemantixHomeDirUsesExactGlobalHookPath(t *testing.T) {
	home := t.TempDir()
	semantixHome := filepath.Join(home, "AppData", "Roaming", "semantix")
	settingsPath := filepath.Join(semantixHome, SettingsFilename)
	if err := os.MkdirAll(semantixHome, 0o755); err != nil {
		t.Fatal(err)
	}
	...
	hooks := Load(LoadOptions{HomeDir: home, SemantixHomeDir: semantixHome})
	if len(hooks) != 1 || hooks[0].Command != "echo exact" || hooks[0].Source != settingsPath {
		t.Fatalf("Load hooks = %+v, want exact Semantix home hook", hooks)
	}
}

func TestSemantixHomeDoesNotFallBackToLegacyWhenIsolated(t *testing.T) {
	home := t.TempDir()
	semantixHome := filepath.Join(t.TempDir(), "rx-home")
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	...
	hooks := Load(LoadOptions{})
	if len(hooks) != 0 {
		t.Fatalf("Load hooks = %+v, want empty (isolated SEMANTIX_HOME must not load legacy hooks)", hooks)
	}
...
func TestMatchesToolTranslatesClaudeToolNames(t *testing.T) {
	...
	if !MatchesTool(claude("Bash"), "bash") {
		t.Error(`Claude matcher "Bash" should match Semantix tool "bash"`)
	}
	if !MatchesTool(claude("Write|Edit"), "write_file") {
		t.Error(`Claude matcher "Write|Edit" should match Semantix tool "write_file"`)
	}
	if !MatchesTool(claude("Write|Edit"), "edit_file") {
		t.Error(`Claude matcher "Write|Edit" should match Semantix tool "edit_file"`)
	}
	if MatchesTool(claude("Bash"), "write_file") {
		t.Error(`Claude matcher "Bash" must not match Semantix tool "write_file"`)
	}
	// A native (non-Claude) hook's matcher stays in Semantix's own vocabulary.
	...
	// using either name must still fire against Semantix's "task" tool.
	if !MatchesTool(claude("Agent"), "task") {
		t.Error(`Claude matcher "Agent" (current name) should match Semantix tool "task"`)
	}
	if !MatchesTool(claude("Task"), "task") {
		t.Error(`Claude matcher "Task" (legacy alias) should still match Semantix tool "task"`)
	}
	if !MatchesTool(claude("AskUserQuestion"), "ask") {
		t.Error(`Claude matcher "AskUserQuestion" should match Semantix tool "ask"`)
	}
	for _, name := range []string{"bash_output", "wait"} {
		if !MatchesTool(claude("TaskOutput"), name) || !MatchesTool(claude("BashOutput"), name) {
			t.Errorf(`current "TaskOutput" and legacy "BashOutput" matchers should match Semantix tool %q`, name)
		}
	}
	if !MatchesTool(claude("TaskStop"), "kill_shell") || !MatchesTool(claude("KillShell"), "kill_shell") {
		t.Error(`current "TaskStop" and legacy "KillShell" matchers should match Semantix tool "kill_shell"`)
	}
}
...
func TestClaudeFacingToolNameUsesCurrentNames(t *testing.T) {
	...
	for _, name := range []string{"task", "read_only_task", "parallel_tasks", "explore", "research", "review", "security_review"} {
		...
		if !MatchesTool(claude, name) {
			t.Errorf(`Claude matcher "Agent" should match Semantix tool %q`, name)
		}
		legacy := ResolvedHook{HookConfig: HookConfig{Match: "Task", PayloadFormat: "claude"}, Event: PreToolUse}
		if !MatchesTool(legacy, name) {
			t.Errorf(`legacy Claude matcher "Task" should still match Semantix tool %q`, name)
		}
	}
}
...
// contract ("file_path must be absolute"): a relative Semantix path resolves
...
func TestDecideOutcome(t *testing.T) {
	cases := []struct {
		...
		// same way PreToolUse does; native Semantix PermissionRequest hooks
...
func TestRunClaudePayloadAndDirectArgs(t *testing.T) {
	...
	if payload["tool_name"] != "Bash" {
		t.Fatalf("Claude payload tool_name = %v, want the Claude vocabulary name Bash for Semantix tool bash", payload["tool_name"])
	}
...
// Semantix write_file call and sees the absolute target path Claude's
...
func TestRunClaudeWriteFileGuardFiresAndSeesFilePath(t *testing.T) {
	...
	if input.Command == "" {
		t.Fatal(`matcher "Write" did not fire for Semantix tool "write_file"`)
	}
	var payload map[string]any
	if err := json.Unmarshal([]byte(input.Stdin), &payload); err != nil {
		t.Fatal(err)
	...
	if _, hasPath := toolInput["path"]; hasPath {
		t.Fatalf("tool_input still has Semantix's \"path\" key: %#v", toolInput)
	}
}

// TestRunClaudeAgentGuardFiresAndSeesRequiredFields covers the full matcher to
// stdin path for a dedicated Semantix subagent wrapper. Claude Agent requires
...
func TestRunClaudeAgentGuardFiresAndSeesRequiredFields(t *testing.T) {
	...
	if input.Command == "" {
		t.Fatal(`matcher "Agent" did not fire for Semantix tool "security_review"`)
	}
	var payload map[string]any
	if err := json.Unmarshal([]byte(input.Stdin), &payload); err != nil {
		t.Fatal(err)
