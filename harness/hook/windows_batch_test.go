...
func TestProjectAndGlobalExtensionlessHooksUseAutoResolvedBash(t *testing.T) {
	...
	writeHookTestFile(t, filepath.Join(workspace, ".semantix", "settings.json"), `{"hooks":{"SessionStart":[{"command":"hooks/project-start"}]}}`)
	semantixHome := filepath.Join(home, ".semantix")
	writeHookTestFile(t, filepath.Join(semantixHome, "settings.json"), `{"hooks":{"SessionStart":[{"command":"hooks/global-start"}]}}`)
