...
func Diagnose(ctx context.Context, opts DiagnoseOptions) (DiagnosticReport, error) {
	...
	for _, check := range configReport.Checks {
		if check.Exists && !check.Valid {
			valid = false
			report.add("error", "config.invalid_toml", check.Scope, "Configuration cannot be parsed: "+check.Error, "Run semantix-agent doctor repair; add --project only for a project config.")
		}
	}
	...
	checkDirectoryMode(&report, config.SemantixHomeDir(), "Semantix home")
	if config.MemoryUserDir() != config.SemantixHomeDir() {
		checkDirectoryMode(&report, config.MemoryUserDir(), "Semantix state directory")
	}
	...
	if err != nil {
		report.add("error", "config.load_failed", "runtime", err.Error(), "Run semantix-agent doctor repair, then inspect the reported global or project configuration.")
		return report, nil
	}
...
func validateProviders(report *DiagnosticReport, cfg *config.Config) {
	...
	for i := range cfg.Providers {
		...
		if entry.Kind != "openai" && entry.Kind != "anthropic" {
			report.add("error", "provider.unsupported_kind", scope, fmt.Sprintf("Provider kind %q is not registered by packaged Semantix builds.", entry.Kind), "Use openai or anthropic compatibility.")
		}
		...
		if entry.APIKeyEnv != "" && !config.IsValidCredentialKey(entry.APIKeyEnv) {
			...
			report.add("warning", "provider.missing_key", scope, "The configured API key is missing from the global Semantix credential file.", "Add the key in Settings or <Semantix home>/.env.")
		}
	}
...
func checkDirectoryMode(report *DiagnosticReport, path, label string) {
	if path == "" {
		report.add("warning", "directory.unavailable", label, label+" path is unavailable.", "Restore the user home or Semantix path environment configuration.")
		return
	}
	st, err := os.Stat(path)
	if err != nil {
...
func checkDerivedJSON(report *DiagnosticReport) {
	...
	for name, path := range paths {
		...
		if !json.Valid(b) {
			report.add("warning", "derived.invalid_json", "derived:"+name, fmt.Sprintf("Derived desktop state %s is malformed.", filepath.Base(path)), "Run semantix-agent doctor repair, or delete the broken derived file after backing it up.")
		}
	}
}
		...
		if err == nil {
			...
			if callErr != nil {
				...
				case resp.StatusCode == http.StatusUnauthorized || resp.StatusCode == http.StatusForbidden:
					report.add("error", "network.authentication_failed", "provider:"+entry.Name, fmt.Sprintf("Provider rejected credentials with HTTP %d.", resp.StatusCode), "Update the provider credential in Semantix Settings.")
