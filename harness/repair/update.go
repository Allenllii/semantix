...
func lockPendingUpdateStrict() (func(), error) {
	...
	if path == "" {
		return nil, fmt.Errorf("pending update: Semantix state directory is unavailable")
	}
...
func PrepareFileUpdate(fromVersion, toVersion, targetPath string, siblingPaths ...string) (*UpdateTransaction, error) {
	...
	root := config.MemoryUserDir()
	if root == "" {
		return nil, fmt.Errorf("prepare update: Semantix state directory is unavailable")
	}
...
func PrepareAppBundleUpdateHandoff(fromVersion, toVersion, appPath, backupPath, stagedAppPath, stagingPath string, ownerPID int) (*UpdateTransaction, error) {
	...
	// An uncooperative writer is not covered by Semantix's mutation lock. Recheck
...
func newAppBundleUpdateTransaction(fromVersion, toVersion, appPath, backupPath string) (*UpdateTransaction, error) {
	...
	if !strings.HasSuffix(strings.ToLower(tx.TargetPath), ".app") ||
		tx.BackupPath != tx.TargetPath+".semantix-update-backup" {
		return nil, fmt.Errorf("prepare update: invalid macOS bundle paths")
	}
	return tx, nil
}
...
func PublishClaimedFileUpdateMemberExact(
	...
	if !member.MissingBefore {
		...
		retained = member.TargetPath + ".semantix-update-aside-" + transactionID[:16]
...
func stageFileUpdateContent(targetPath string, content []byte, mode os.FileMode) (string, string, string, error) {
	tmp, err := os.CreateTemp(filepath.Dir(targetPath), "."+filepath.Base(targetPath)+".semantix-update-stage-*")
	if err != nil {
		return "", "", "", err
	}
	path := tmp.Name()
...
func quarantineExistingAppBundleUpdateBackup(tx *UpdateTransaction) (string, string, error) {
	...
	if err != nil {
		return "", "", fmt.Errorf("resolve current Semantix executable: %w", err)
	}
	...
	if err != nil || rel == "." || rel == ".." || strings.HasPrefix(rel, ".."+string(filepath.Separator)) {
		return "", "", fmt.Errorf("existing handoff backup is outside the current Semantix installation")
	}
...
// a path merely because its name resembles a Semantix quarantine.
...
func writePendingUpdate(tx *UpdateTransaction, createOnly bool) error {
	...
	path := PendingUpdatePath()
	if path == "" {
		return fmt.Errorf("pending update: Semantix state directory is unavailable")
	}
...
func classifyPendingUpdate() (pendingUpdateDisposition, *UpdateTransaction, error) {
	path := PendingUpdatePath()
	if path == "" {
		return pendingUpdateNone, nil, fmt.Errorf("Semantix state directory is unavailable")
	}
...
// recovery needs regardless of where Semantix is installed: which release it
...
func quarantinePendingUpdate(reason string) (string, error) {
	path := PendingUpdatePath()
	if path == "" {
		return "", fmt.Errorf("Semantix state directory is unavailable")
	}
...
func rollbackPendingUpdateMatchingLocked(
	...
	// SEMANTIX_HOME profiles cannot quarantine or restore the same binaries
	...
	case "app-bundle":
		...
		if _, statErr := os.Lstat(tx.TargetPath); statErr == nil {
			...
			var retainErr error
			failed, retainErr = retainUpdateRollbackNode(tx.TargetPath, "semantix-failed")
			if retainErr != nil {
...
func restoreReleaseUnit(
	...
	for i, f := range files {
		aside := f.TargetPath + ".semantix-rollback-aside"
...
func allowedUpdateTargetBase(base string, primary bool) bool {
	...
	case "semantix-agent-desktop", "semantix-agent-desktop.exe":
		return primary
	case "semantix-agent.exe", "semantix.exe":
		// semantix-agent.exe is the Windows CLI; semantix.exe is the
		// display-titled desktop sibling (brand.Name + ".exe").
		return !primary
	case "semantix-agent", "semantix-agent-guard", "semantix-agent-guard.exe", "semantix-agent-launcher.exe", "semantix-agent-update-helper.exe", "semantix-agent-cli.exe":
		return !primary
	default:
		return false
	}
...
func validateUpdateTransactionForLauncher(tx *UpdateTransaction, launcher string) error {
	...
	case "file":
		if !allowedUpdateTargetBase(filepath.Base(tx.TargetPath), true) {
			return fmt.Errorf("pending update target is not a Semantix executable")
		}
	...
	case "app-bundle":
		if !strings.HasSuffix(strings.ToLower(tx.TargetPath), ".app") || tx.BackupPath != tx.TargetPath+".semantix-update-backup" {
...
func validateAppBundleHandoffMetadata(tx *UpdateTransaction) error {
	...
	if !strings.HasPrefix(stagingBase, "semantix-mac-update-") {
...
func validateOrphanedAppBundleBackupMetadata(tx *UpdateTransaction) error {
	...
	prefix := filepath.Base(tx.BackupPath) + ".semantix-orphaned-"
