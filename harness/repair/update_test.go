...
func TestFileUpdateRollbackRestoresPreviousBinary(t *testing.T) {
	...
	target := filepath.Join(t.TempDir(), "semantix-agent-desktop")
	originalExecutable := repairExecutable
	repairExecutable = func() (string, error) { return filepath.Join(filepath.Dir(target), "semantix-agent-guard"), nil }
...
func TestReconcilePendingFileUpdateQuarantinesForeignTarget(t *testing.T) {
	...
	repairExecutable = func() (string, error) { return filepath.Join(t.TempDir(), "semantix-agent-guard"), nil }
...
func TestPrepareFileUpdateQuarantinesForeignTarget(t *testing.T) {
	...
	repairExecutable = func() (string, error) { return filepath.Join(newLauncherDir, "semantix-agent-guard"), nil }
	newTarget := filepath.Join(newLauncherDir, "semantix-agent-desktop")
...
func TestAppBundleRollbackRestoresWhenLiveBundleIsMissing(t *testing.T) {
	...
	app := filepath.Join(dir, "Semantix.app")
	exe := filepath.Join(app, "Contents", "MacOS", "Semantix")
...
func TestMarkUpdateHealthyRejectsAppBundleDrift(t *testing.T) {
	...
	stagedExe := filepath.Join(stagedApp, "Contents", "MacOS", "Semantix")
...
func TestAppBundleRollbackRejectsChangedBackupTree(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(backup, "Contents", "MacOS", "Semantix"), []byte("tampered"), 0o700); err != nil {
		t.Fatal(err)
	}
...
func TestAppBundleRollbackCompensatesBackupChangedDuringPublish(t *testing.T) {
	...
	rollbackSwapRename = func(oldPath, newPath string) error {
		...
		if oldPath == backup && newPath == app {
			return os.WriteFile(filepath.Join(app, "Contents", "MacOS", "Semantix"), []byte("tampered-during-publish"), 0o700)
		}
		return nil
	}
	...
	rejected, err := filepath.Glob(app + ".semantix-cleanup-*")
	if err != nil || len(rejected) != 1 {
		t.Fatalf("rejected backup bundle = %v, %v", rejected, err)
	}
	rejectedExe := filepath.Join(rejected[0], "Contents", "MacOS", "Semantix")
...
func TestLegacyAppBundleRollbackWithoutTreeDigest(t *testing.T) {
	...
	backupExe := filepath.Join(backup, "Contents", "MacOS", "Semantix")
...
func TestRepairPlanRejectsLegacyAppBundleSymlinkBackupBeforeMutation(t *testing.T) {
	...
	if got, err := os.ReadFile(filepath.Join(external, "Contents", "MacOS", "Semantix")); err != nil || string(got) != "old" {
...
func TestRollbackPendingUpdateRejectsUnexpectedVersion(t *testing.T) {
	...
	target := filepath.Join(dir, "semantix-agent-desktop")
	originalExecutable := repairExecutable
	repairExecutable = func() (string, error) { return filepath.Join(dir, "semantix-agent-guard"), nil }
	t.Cleanup(func() { repairExecutable = originalExecutable })
...
func TestRollbackPendingUpdateFailsClosedWhenStateLockIsUnavailable(t *testing.T) {
	...
	target := filepath.Join(dir, "semantix-agent-desktop")
	guard := filepath.Join(dir, "semantix-agent-guard")
	originalExecutable := repairExecutable
...
func TestFileUpdateRollbackRestoresReleaseUnit(t *testing.T) {
	...
	missingSibling := filepath.Join(dir, "semantix-agent-update-helper.exe")
...
func TestClearUpdateApplyFailureExactPreservesDifferentTransactionMarker(t *testing.T) {
	...
	first := &UpdateTransaction{
		...
		TargetKind:    "file",
		TargetPath:    filepath.Join(t.TempDir(), "semantix-agent-desktop"),
...
func TestRecordClaimedFileUpdateInstalledBindsOptionalMissingSibling(t *testing.T) {
	...
	target := filepath.Join(dir, "semantix-agent-desktop")
	guard := filepath.Join(dir, "semantix-agent-guard")
	optionalCLI := filepath.Join(dir, "semantix-agent")
	originalExecutable := repairExecutable
	repairExecutable = func() (string, error) { return guard, nil }
	t.Cleanup(func() { repairExecutable = originalExecutable })
	for path, body := range map[string]string{target: "old-desktop", guard: "old-guard"} {
...
func TestHealthyAppUpdateRejectsBackupDrift(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(backup, "Contents", "MacOS", "Semantix"), []byte("unrelated"), 0o700); err != nil {
		t.Fatal(err)
	}
	...
	if got, err := os.ReadFile(filepath.Join(backup, "Contents", "MacOS", "Semantix")); err != nil || string(got) != "unrelated" {
...
func TestFileUpdateRollbackCompensatesOnPartialFailure(t *testing.T) {
	...
	target := filepath.Join(dir, "semantix-agent-desktop")
	guard := filepath.Join(dir, "semantix-agent-guard")
	added := filepath.Join(dir, "semantix-agent-update-helper.exe")
	originalExecutable := repairExecutable
	repairExecutable = func() (string, error) { return guard, nil }
	t.Cleanup(func() { repairExecutable = originalExecutable })
	...
	rollbackPublishStage = func(oldpath, newpath string) error {
		if strings.HasPrefix(oldpath, guard+".semantix-rollback-stage-") && newpath == guard {
	...
	leftovers, err := filepath.Glob(filepath.Join(dir, "*.semantix-rollback-*"))
...
func TestFileUpdateRollbackPreservesAsideReplacedBeforeCleanup(t *testing.T) {
	...
	rollbackPublishStage = func(oldpath, newpath string) error {
		...
		return os.WriteFile(newpath+".semantix-rollback-aside", []byte("concurrent-aside"), 0o700)
	}
	t.Cleanup(func() { rollbackPublishStage = originalPublish })
	...
	aside := target + ".semantix-rollback-aside"
...
func TestFileUpdateRollbackRetryPreservesRetainedNewBinary(t *testing.T) {
	...
	targetAside := target + ".semantix-rollback-aside"
...
func TestFileUpdateRollbackRechecksWholeReleaseUnitBeforeCommit(t *testing.T) {
	...
	injected := false
	rollbackSwapRename = func(oldPath, newPath string) error {
		if !injected && oldPath == guard && newPath == guard+".semantix-rollback-aside" {
			injected = true
	...
	if got, err := os.ReadFile(target + ".semantix-rollback-aside"); err != nil || string(got) != "new-desktop" {
...
func TestRollbackPendingUpdateStateRejectsDriftInsideRetainRename(t *testing.T) {
	...
	injected := false
	rollbackSwapRename = func(oldpath, newpath string) error {
		if !injected && oldpath == target && newpath == target+".semantix-rollback-aside" {
			injected = true
...
func TestRollbackPendingAppBundleStateRejectsDriftInsideRetainRename(t *testing.T) {
	...
	injected := false
	rollbackSwapRename = func(oldpath, newpath string) error {
		if !injected && oldpath == app && strings.Contains(newpath, ".semantix-failed-") {
			injected = true
...
func TestFileUpdateRollbackStageFailureLeavesInstallUntouched(t *testing.T) {
	...
	originalCopy := rollbackStageCopy
	rollbackStageCopy = func(src, dst string, mode os.FileMode) (string, error) {
		if strings.HasPrefix(dst, guard+".semantix-rollback-stage-") {
...
func TestPrepareFileUpdateDoesNotOverwriteLegacyBackupName(t *testing.T) {
	...
	legacyBackup := filepath.Join(backupDir, "semantix-agent-desktop.previous")
...
func TestFileUpdateRollbackBypassesAndPreservesCrashedStage(t *testing.T) {
	...
	stage := target + ".semantix-rollback-stage-crashed"
...
func TestRetainUpdateRollbackNodeRetriesNameCollision(t *testing.T) {
	target := filepath.Join(t.TempDir(), "Semantix.app")
	...
	retained, err := retainUpdateRollbackNode(target, "semantix-failed")
	if err != nil {
		t.Fatal(err)
	}
	if calls != 2 || !strings.Contains(retained, ".semantix-failed-") {
...
func TestPublishClaimedFileUpdateMemberUsesCompareAndPublish(t *testing.T) {
	...
	aside := target + ".semantix-update-aside-" + UpdateTransactionID(claimed)[:16]
...
func TestPublishClaimedFileUpdateMemberRestoresPreparedFileWhenStageSwapped(t *testing.T) {
	...
	fileUpdateAfterRetain = func(_, _ string) {
		stages, err := filepath.Glob(filepath.Join(dir, ".semantix-agent-desktop.semantix-update-stage-*"))
		if err != nil {
			hookErr = err
			return
		}
	...
	if err == nil || !strings.Contains(err.Error(), "installed semantix-agent-desktop changed") {
	...
	rejected, err := filepath.Glob(target + ".semantix-cleanup-*")
	if err != nil {
		t.Fatal(err)
	}
	if len(rejected) != 1 {
...
func TestFileUpdateRollbackCleansInstalledNodeBoundToTransaction(t *testing.T) {
	...
	if _, err := os.Stat(target + ".semantix-rollback-aside"); !os.IsNotExist(err) {
...
func TestAppBundleRollbackPreservesLiveBundleWithoutReplacementOwnership(t *testing.T) {
	...
	failed, err := filepath.Glob(app + ".semantix-failed-*")
	if err != nil || len(failed) != 1 {
		t.Fatalf("retained unbound bundle = %v, %v", failed, err)
	}
	if got, err := os.ReadFile(filepath.Join(failed[0], "Contents", "MacOS", "Semantix")); err != nil || string(got) != "unbound-live" {
