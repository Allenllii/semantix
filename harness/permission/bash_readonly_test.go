...
func TestIsReadOnlyBashSubject(t *testing.T) {
	tests := []struct {
		...
		{"awk 'BEGIN{system(\"touch /tmp/semantix-awk-test\")}'", false},
		{"awk 'BEGIN{print \"evil\" > \"/tmp/semantix-awk-test\"}'", false},
		{"sed 's/a/b/' file.txt", false},
		{"sed -e 's/.*/touch \\/tmp\\/semantix-sed-test/e' file.txt", false},
