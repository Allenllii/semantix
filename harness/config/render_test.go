...
func isolateUserConfigHome(t *testing.T) string {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	for _, key := range []string{"SEMANTIX_HOME", "SEMANTIX_STATE_HOME", "SEMANTIX_CACHE_HOME"} {
		t.Setenv(key, "")
...
func setRuntimeGOOS(t *testing.T, goos string) {
	t.Helper()
	t.Setenv("SEMANTIX_TEST_GOOS", goos)
	old := runtimeGOOS
	runtimeGOOS = goos
	t.Cleanup(func() { runtimeGOOS = old })
}

func expectedDefaultSemantixHome(home string) string {
...
func TestUserConfigDisplayPathCollapsesHome(t *testing.T) {
	...
	if !strings.HasSuffix(got, "semantix/config.toml") {
		t.Fatalf("display path = %q, want semantix/config.toml suffix", got)
	}
...
func TestUserConfigPathUsesSemantixHome(t *testing.T) {
	home := isolateUserConfigHome(t)
	want := filepath.Join(expectedDefaultSemantixHome(home), "config.toml")
	if got := UserConfigPath(); filepath.Clean(got) != filepath.Clean(want) {
		t.Fatalf("UserConfigPath() = %q, want %q", got, want)
	}
}

func TestSemantixManagedConfigPathsAreConfigFilesOnly(t *testing.T) {
	home := isolateUserConfigHome(t)
	...
	// the sensitive Semantix-home siblings (credentials, hooks, skills,
	...
	for _, got := range paths {
		...
		for _, forbidden := range []string{
			...
			SemantixHomeDir(),
			UserCredentialsPath(),
			filepath.Join(SemantixHomeDir(), "settings.json"),
			filepath.Join(SemantixHomeDir(), "skills"),
			filepath.Join(SemantixHomeDir(), "sessions"),
		} {
...
func TestUserConfigPathHonorsSemantixHome(t *testing.T) {
	home := isolateUserConfigHome(t)
	custom := filepath.Join(home, "custom-home")
	t.Setenv("SEMANTIX_HOME", custom)
...
func TestLoadForRootUsesWindowsHomeFallbackWhenConfigDirUnavailable(t *testing.T) {
	...
	configPath := filepath.Join(home, "AppData", "Roaming", "semantix", "config.toml")
...
func TestRenderTOMLRoundTrips(t *testing.T) {
	...
	orig.Network = NetworkConfig{
		...
		Proxy: NetworkProxyConfig{
			...
			Port:     7890,
			Username: "user",
			Password: "${SEMANTIX_PROXY_PASSWORD}",
		},
	}
	...
	orig.Bot.Feishu.OutboundMediaRoots = []string{"/tmp/semantix-media", "/srv/shots"}
	orig.Bot.Routes = []BotRouteConfig{{
		...
		WorkspaceRoot:    "/tmp/semantix-route",
	}}
	...
	orig.Bot.Connections = []BotConnectionConfig{{
		...
		WorkspaceRoot:    "/tmp/semantix-bot",
	...
	orig.Plugins = []PluginEntry{
		{Name: "example", Command: "semantix-plugin-example"},
	...
	if len(got.Bot.Connections) != 1 || got.Bot.Connections[0].Model != "deepseek-pro" || got.Bot.Connections[0].WorkspaceRoot != "/tmp/semantix-bot" {
	...
	if len(got.Bot.Feishu.OutboundMediaRoots) != 2 || got.Bot.Feishu.OutboundMediaRoots[0] != "/tmp/semantix-media" {
		t.Errorf("feishu outbound_media_roots not preserved: %+v", got.Bot.Feishu.OutboundMediaRoots)
	}
	if len(got.Bot.Routes) != 1 || got.Bot.Routes[0].WorkspaceRoot != "/tmp/semantix-route" || got.Bot.Routes[0].ChatID != "oc_group" {
	...
	if len(got.Bot.Connections[0].SessionMappings) != 1 || got.Bot.Connections[0].SessionMappings[0].Scope != "project" || got.Bot.Connections[0].SessionMappings[0].WorkspaceRoot != "/tmp/semantix-bot" {
...
func TestRenderTOMLRoundTripsProviderHeadersAndModelOverrides(t *testing.T) {
	...
	rendered := RenderTOML(orig)
	if !strings.Contains(rendered, `headers     = { HTTP-Referer = "https://app.example", X-Title = "Semantix" }`) {
	...
	if p.Headers["HTTP-Referer"] != "https://app.example" || p.Headers["X-Title"] != "Semantix" {
...
func TestIsolatedHomeDirReturnsCleanPath(t *testing.T) {
	raw := filepath.Join(t.TempDir(), "isolated-semantix")
	t.Setenv("SEMANTIX_HOME", raw)
	got := IsolatedHomeDir()
...
func TestLegacyOSSupportDirEmptyWhenIsolated(t *testing.T) {
	isolateUserConfigHome(t)
	t.Setenv("SEMANTIX_HOME", filepath.Join(t.TempDir(), "isolated-home"))
...
func TestCacheDirHonorsSemantixHome(t *testing.T) {
	home := t.TempDir()
	isolated := filepath.Join(home, "isolated-home")
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	t.Setenv("SEMANTIX_HOME", isolated)
...
func TestCacheDirHonorsSemantixCacheHomeOverSemantixHome(t *testing.T) {
	home := t.TempDir()
	cacheHome := filepath.Join(home, "custom-cache")
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	t.Setenv("SEMANTIX_HOME", filepath.Join(home, "isolated-home"))
	t.Setenv("SEMANTIX_CACHE_HOME", cacheHome)

	got := CacheDir()
	want := cacheHome
	if filepath.Clean(got) != filepath.Clean(want) {
		t.Fatalf("CacheDir() = %q, want %q (SEMANTIX_CACHE_HOME must win)", got, want)
	}
}
...
func TestUserConfigLoadPathNoLegacyFallbackWhenIsolated(t *testing.T) {
	...
	productionHome := expectedDefaultSemantixHome(home)
...
func TestMigrateLegacyIfNeededSkipsWhenIsolated(t *testing.T) {
	...
	legacyDir := filepath.Join(home, ".semantix")
...
// security control: a cloned repository's semantix-agent.toml must not be able to
...
func TestProjectConfigCannotOverrideSecrets(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(project, "semantix-agent.toml"), []byte(projectTOML), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if cfg.Secrets.FilterSubprocessEnv {
		t.Error("project semantix-agent.toml enabled filter_subprocess_env; [secrets] must stay user-global")
	}
	if cfg.Secrets.ProtectSensitiveFiles {
		t.Error("project semantix-agent.toml enabled protect_sensitive_files; [secrets] must stay user-global")
	}
}
