...
func userConfigDir() string {
	return semantixHomeDir()
}

func semantixHomeDir() string {
	if dir := cleanEnvDir("SEMANTIX_HOME"); dir != "" {
		return dir
	}
	if runtimeGOOS == "windows" {
		if dir := osUserConfigDir(); dir != "" {
			return filepath.Join(dir, "semantix")
		}
		if home, err := osUserHomeDir(); err == nil && home != "" {
			return filepath.Join(home, "AppData", "Roaming", "semantix")
		}
		return ""
	}
	if home, err := osUserHomeDir(); err == nil && home != "" {
		return filepath.Join(home, ".semantix")
	}
	if dir := osUserConfigDir(); dir != "" {
		return filepath.Join(dir, "semantix")
	}
...
func legacyXDGConfigPaths() []string {
	...
	if dir := cleanEnvDir("XDG_CONFIG_HOME"); dir != "" {
		add(filepath.Join(dir, "semantix", "config.toml"))
	}
	if home, err := osUserHomeDir(); err == nil && home != "" {
		add(filepath.Join(home, ".config", "semantix", "config.toml"))
	}
	return paths
}

func userSupportDir() string {
	if dir := cleanEnvDir("SEMANTIX_STATE_HOME"); dir != "" {
		return dir
	}
	return semantixHomeDir()
}
...
func legacyOSSupportDir() string {
	...
	path := filepath.Join(dir, "semantix")
	if current := semantixHomeDir(); current != "" && samePath(path, current) {
		return ""
	}
	return path
}
...
func userCacheDir() string {
	if dir := cleanEnvDir("SEMANTIX_CACHE_HOME"); dir != "" {
		return dir
	}
	if dir := cleanEnvDir("SEMANTIX_HOME"); dir != "" {
...
// IsolatedHomeDir returns the SEMANTIX_HOME directory when it has been
...
func IsolatedHomeDir() string {
	return cleanEnvDir("SEMANTIX_HOME")
}
...
// real location instead of a hardcoded ~/.semantix path.
func userConfigDisplayPath() string {
	p := userConfigPath()
	if p == "" {
		return "<os-config-dir>/semantix/config.toml"
	}
	if home, err := osUserHomeDir(); err == nil && home != "" {
...
// UserConfigPath is the user-global config.toml. It lives under Semantix home:
// SEMANTIX_HOME/config.toml, then ~/.semantix/config.toml on Unix-like systems,
// or %AppData%/semantix/config.toml on Windows. If %AppData% is unavailable on
// Windows, it falls back to %USERPROFILE%/AppData/Roaming/semantix/config.toml.
...
// from the current v1.8.1 Semantix-home config path.
...
func SemantixManagedConfigPaths() []string {
	var out []string
...
// SemantixHomeDir is the current Semantix home directory. It honors
// SEMANTIX_HOME, then uses ~/.semantix on macOS/Linux or %APPDATA%/semantix on
// Windows, with a %USERPROFILE%/AppData/Roaming fallback when %APPDATA% is
// unavailable.
func SemantixHomeDir() string { return semantixHomeDir() }

// RemoteStateDir is local state for the remote-SSH module (the managed
// known_hosts file, cached host metadata): <Semantix home>/remote. Routed
// through the home resolver so SEMANTIX_HOME isolation holds.
...
// RemoteKnownHostsPath is the Semantix-managed known_hosts file (OpenSSH
...
// missing tool-call thinking recovery gate (#7059): <Semantix home>/state. The
...
func WorkspaceLeaseDir() string {
	// Deliberately ignore SEMANTIX_HOME/SEMANTIX_CACHE_HOME here. Two app
	...
	return filepath.Join(dir, "semantix", "workspace-leases")
}

// RepairMutationLockDir stores target-path repair locks in the OS-user cache.
// It deliberately ignores Semantix home/cache overrides: isolated instances
// can still repair the same project semantix-agent.toml, so their locks must converge.
func RepairMutationLockDir() string {
	dir := osUserCacheDir()
	if strings.TrimSpace(dir) == "" {
		return ""
	}
	return filepath.Join(dir, "semantix", "repair-mutation-locks")
}
...
// profile; other platforms keep using Semantix state storage.
func DeliveryWorktreeDir() string {
	...
	if runtimeGOOS == "windows" {
		if dir := osUserCacheDir(); dir != "" {
			return filepath.Join(dir, "semantix", "worktrees")
		}
		if home, err := osUserHomeDir(); err == nil && home != "" {
			return filepath.Join(home, "AppData", "Local", "semantix", "worktrees")
		}
		return ""
	}
	dir := userSupportDir()
...
// UserCredentialsPath is the semantix-owned global .env file under Semantix
// home. It is the single source for provider credentials saved by Semantix, so
// stale shell, Windows, project, or home env vars cannot silently override keys
// the user saved through setup or settings. "" when Semantix home can't be
// resolved.
...
// Used by `semantix-agent --continue` / `--resume` to find the recent ones. Empty
...
// MemoryUserDir returns the semantix user state root (…/semantix), under which
// the user-global SEMANTIX.md and the per-project auto-memory store live. Empty
...
// commands), in canonical-first order. .semantix is ours; .agents / .agent /
...
// .semantix/settings.json (see internal/hook).
var ConventionDirs = []string{".semantix", ".agents", ".agent", ".claude"}

// conventionSubdirsAsc joins sub under each ConventionDir of base, in ascending
// priority (reverse of ConventionDirs) so the canonical .semantix ends up the
...
// ~/.semantix/commands), the Semantix home commands dir, the legacy OS
// app-support dir if different, then the project's
// convention dirs (.claude/commands … .semantix/commands). Scanning the .claude /
