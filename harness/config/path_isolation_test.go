...
// the state home is SEMANTIX_STATE_HOME when set and SEMANTIX_HOME otherwise.
...
func TestStatsDirResolution(t *testing.T) {
	...
	if got := StatsDir(); got != filepath.Join(home, "stats") {
		t.Fatalf("SEMANTIX_HOME stats dir: want %q, got %q", filepath.Join(home, "stats"), got)
	}
	t.Setenv("SEMANTIX_STATE_HOME", state)
	if got := StatsDir(); got != filepath.Join(state, "stats") {
		t.Fatalf("SEMANTIX_STATE_HOME stats dir: want %q, got %q", filepath.Join(state, "stats"), got)
	}
}
