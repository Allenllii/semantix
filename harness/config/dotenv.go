...
// loadDotEnv loads Semantix's global .env for provider credentials. The
...
// expansion, then loads Semantix's global .env for provider credentials.
...
// other and project files cannot redirect Semantix's own config/credential
// paths.
...
func isProjectDotEnvControlKey(key string) bool {
	...
	if strings.HasPrefix(upper, "SEMANTIX_") {
		return true
	}
	switch upper {
