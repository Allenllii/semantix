...
// separate from the `semantix-agent setup` wizard (cli) so a GUI can apply one setting at a
...
// SetLanguage pins the CLI UI/model language; empty/auto clears the override so runtime detection falls back to SEMANTIX_LANG / locale.
...
func (c *Config) SetCLIUpdateChannel(channel string) error {
	...
	case "", "stable", "preview", "canary", "beta", "next":
		c.CLI.UpdateChannel = ""
	default:
		return fmt.Errorf("CLI update channel %q is unsupported; Semantix now uses the official release channel", channel)
	}
	return nil
}
...
// .mcp.json-only server would otherwise be serialized into semantix-agent.toml or the
...
// action cannot drift to another project's semantix-agent.toml or .mcp.json after the
...
func ClearPluginAuthenticationInSourceForRoot(root, name string) (PluginEntry, bool, string, error) {
	...
	if resolvedRoot != "." {
		projectTOML = filepath.Join(resolvedRoot, "semantix-agent.toml")
...
func pluginTOMLSourcePathForRoot(root, name string) string {
	...
	if resolved := resolveRoot(root); resolved != "." {
		projectTOML = filepath.Join(resolved, "semantix-agent.toml")
	}
...
// and legacy JSON receives a disable marker for older Semantix versions.
...
// half-written semantix-agent.toml that fails to parse on next load. Parent directories
// are created as needed.
//
// For project configs (./semantix-agent.toml) the write is incremental: only sections
...
func SaveMinimalProjectReasoningLanguage(path, lang string) (string, error) {
	...
	body := fmt.Sprintf(`# Semantix project configuration.
...
// IsUserConfigPath reports whether path is one of Semantix's current or legacy
...
// (SourcePath), or to ./semantix-agent.toml when none exists yet — the conventional
...
// user's global config when root has no semantix-agent.toml. Existing project files
