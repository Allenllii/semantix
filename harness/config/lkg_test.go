...
func TestLoadForRootIsolatesBrokenProjectConfig(t *testing.T) {
	...
	project := t.TempDir()
	if err := os.WriteFile(filepath.Join(project, "semantix-agent.toml"), []byte("[broken\n"), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestLoadForRootTypeErrorDoesNotPartiallyApplyProjectConfig(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(project, "semantix-agent.toml"), []byte(body), 0o600); err != nil {
		t.Fatal(err)
	}
	cfg, err := LoadForRoot(project)
	if err != nil {
