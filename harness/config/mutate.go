...
// other Semantix processes cannot save stale snapshots over one another.
...
// with both other goroutines and other Semantix processes. The cross-process
...
// Semantix credential environment.
...
// two Semantix processes editing overlapping config-source sets from deadlocking.
func lockConfigFilesEdits(paths ...string) (func(), error) {
	userEditMu.Lock()
...
func configEditLockRegistryDir() (string, error) {
	...
	if runtime.GOOS != "windows" {
		...
		return filepath.Join(string(filepath.Separator), "tmp", fmt.Sprintf("semantix-config-locks-%x", digest[:8])), nil
	}
	home := strings.TrimSpace(current.HomeDir)
	if home == "" {
		return "", fmt.Errorf("lock config edits: OS user home unavailable")
	}
	return filepath.Join(filepath.Clean(home), ".semantix", "locks", fmt.Sprintf("config-edits-%x", digest[:8])), nil
}
