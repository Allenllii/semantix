...
func TestExpandVars(t *testing.T) {
	t.Setenv("SEMANTIX_TEST_TOKEN", "sk-123")
	t.Setenv("SEMANTIX_TEST_EMPTY", "")

	cases := []struct{ in, want string }{
		{"Bearer ${SEMANTIX_TEST_TOKEN}", "Bearer sk-123"},
		{"${SEMANTIX_TEST_MISSING}", ""},                                   // unset, no default → empty
		{"${SEMANTIX_TEST_MISSING:-fallback}", "fallback"},                 // unset → default
		{"${SEMANTIX_TEST_EMPTY:-fallback}", "fallback"},                   // set-but-empty → default
		{"${SEMANTIX_TEST_TOKEN:-fallback}", "sk-123"},                     // set → value, default ignored
		{"no vars here", "no vars here"},                                   // untouched
		{"a${SEMANTIX_TEST_TOKEN}b${SEMANTIX_TEST_MISSING}c", "ask-123bc"}, // multiple refs
	}
	for _, c := range cases {
...
func TestExpandedPlugin(t *testing.T) {
	t.Setenv("SEMANTIX_TEST_KEY", "secret")
	e := PluginEntry{
		Name:    "x",
		Type:    "http",
		URL:     "https://api/${SEMANTIX_TEST_MISSING:-v1}",
		Args:    []string{"--token", "${SEMANTIX_TEST_KEY}"},
		Env:     map[string]string{"K": "${SEMANTIX_TEST_KEY}"},
		Headers: map[string]string{"Authorization": "Bearer ${SEMANTIX_TEST_KEY}"},
	}
	...
	if e.Headers["Authorization"] != "Bearer ${SEMANTIX_TEST_KEY}" {
...
func TestForbidReadRootsForRootResolvesRelativePathsAndScopedEnv(t *testing.T) {
	root := t.TempDir()
	cfg := Default()
	cfg.setExpansionEnv(map[string]string{"SEMANTIX_TEST_SECRET_DIR": "from-dotenv"})
	cfg.Sandbox.ForbidRead = []string{
		"relative-secret",
		"${SEMANTIX_TEST_SECRET_DIR}",
