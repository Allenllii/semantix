...
func TestLockUserConfigEditsSerializesRMW(t *testing.T) {
	...
	path := UserConfigPath()
	if path == "" {
		t.Fatal("UserConfigPath is empty with SEMANTIX_HOME set")
	}
...
func assertUserConfigLockSerializesAcrossProcesses(t *testing.T, firstHome, secondHome, firstTmp, secondTmp string) {
	...
	startHelper := func(mode, processHome, processTmp, started, acquired, release string) (*exec.Cmd, *bytes.Buffer) {
		...
		cmd.Env = testEnvWithOverrides(map[string]string{
			"TMPDIR":                        processTmp,
			"SEMANTIX_HOME":                 processHome,
			"SEMANTIX_CONFIG_LOCK_HELPER":   "1",
			"SEMANTIX_CONFIG_LOCK_MODE":     mode,
			"SEMANTIX_CONFIG_LOCK_STARTED":  started,
			"SEMANTIX_CONFIG_LOCK_ACQUIRED": acquired,
			"SEMANTIX_CONFIG_LOCK_RELEASE":  release,
		})
...
func TestLockUserConfigEditsHelperProcess(t *testing.T) {
	...
	started := os.Getenv("SEMANTIX_CONFIG_LOCK_STARTED")
	acquired := os.Getenv("SEMANTIX_CONFIG_LOCK_ACQUIRED")
	release := os.Getenv("SEMANTIX_CONFIG_LOCK_RELEASE")
	...
	switch os.Getenv("SEMANTIX_CONFIG_LOCK_MODE") {
	case "bot":
		cfg.Bot.Connections = []BotConnectionConfig{{
			ID:       "cross-process",
			Provider: "qq",
...
func TestConfigEditLockCanonicalizesAliasesAndIgnoresCacheOverrides(t *testing.T) {
	...
	link := filepath.Join(dir, "semantix-agent.toml")
	...
	t.Setenv("SEMANTIX_CACHE_HOME", filepath.Join(dir, "cache-a"))
	first, err := configFileEditLockPath(link)
	if err != nil {
		t.Fatal(err)
	}
	t.Setenv("SEMANTIX_CACHE_HOME", filepath.Join(dir, "cache-b"))
	...
	t.Setenv("SEMANTIX_HOME", filepath.Join(dir, "semantix-home"))
