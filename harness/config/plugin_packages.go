...
// in <Semantix home>/plugin-packages.json so uninstall/disable can remove the
// entire bundle without editing user-authored config.
func mergeInstalledPluginPackages(cfg *Config, root string) []string {
	if cfg == nil {
		return nil
	}
	semantixHome := SemantixHomeDir()
	if strings.TrimSpace(semantixHome) == "" {
		return nil
	}
	installed, warnings := pluginpkg.LoadInstalled(semantixHome)
...
func pluginPackageCommandRoots() []command.Root {
	...
	installed, _ := pluginpkg.LoadInstalled(semantixHome)
...
func pluginPackageEnv(installed pluginpkg.InstalledPlugin, root, workspaceRoot string, env map[string]string) map[string]string {
	...
	out["SEMANTIX_PLUGIN_ROOT"] = root
	out["SEMANTIX_PLUGIN_NAME"] = installed.Name
	out["CLAUDE_PLUGIN_ROOT"] = root
	out["CLAUDE_PROJECT_DIR"] = workspaceRoot
	out["SEMANTIX_WORKSPACE_ROOT"] = workspaceRoot
	if installed.Version != "" {
		out["SEMANTIX_PLUGIN_VERSION"] = installed.Version
	}
	return out
}
...
func pluginPackageEntriesEqual(a, b PluginEntry) bool {
	...
	for _, env := range []map[string]string{a.Env, b.Env} {
		delete(env, "SEMANTIX_PLUGIN_ROOT")
		delete(env, "SEMANTIX_PLUGIN_NAME")
		delete(env, "SEMANTIX_PLUGIN_VERSION")
