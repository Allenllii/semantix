...
// RemotePasswordCredentialEnvName returns the Semantix-owned credential slot
...
// RemotePassphraseCredentialEnvName returns the Semantix-owned credential slot
...
func remoteCredentialEnvName(hostID, kind string) string {
	...
	return fmt.Sprintf("SEMANTIX_REMOTE_%X_%s", sum[:8], kind)
}

// IsGeneratedRemoteCredential reports whether key is a Semantix-owned slot
...
// EditUserConfigWithCredentials updates config and its Semantix-owned secret
