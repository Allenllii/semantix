...
import (
	...
	"semantix/harness/internal/boot"
	...
	fileencoding "semantix/harness/internal/fileutil/encoding"
	"semantix/harness/internal/i18n"
	"semantix/harness/internal/notify"
	"semantix/harness/internal/provider"
	"semantix/harness/internal/provider/openai"
	"semantix/harness/internal/serve"
	"semantix/harness/internal/sessiontemp"
	"semantix/harness/internal/telemetry"
...
func chatREPL(args []string, version string) int {
	fs := pflag.NewFlagSet("semantix-agent", pflag.ContinueOnError)
	fs.SetInterspersed(true)
	model := fs.String("model", "", "provider name (default: config default_model)")
	profileFlag := fs.String("profile", "", "deprecated: use --preset (economy|balanced|delivery)")
	presetFlag := fs.String("preset", "balanced", "agent execution setting: light | balanced | delivery")
...
func interactiveSetup(configPath, envPath string) int {
	...
	fmt.Print(boxed([]string{
		accent("◆") + " " + fmt.Sprintf(i18n.M.WelcomeTitleFmt, bold("semantix-agent")),
		"",
