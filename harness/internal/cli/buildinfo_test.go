...
func TestBuildInfoSingleLine(t *testing.T) {
	...
	if got, want := info.singleLine(), "semantix-agent v1.2.3"; got != want {
...
func TestBuildInfoVerboseOmitsConfigAndCST(t *testing.T) {
	...
	if !strings.Contains(text, "semantix-agent v9.9.9") {
...
func TestVersionCommandFlags(t *testing.T) {
	...
	if strings.TrimSpace(out) != "semantix-agent v2.0.0" {
