...
func TestCompactionAblationCollapsesTheCachePreservingDeferral(t *testing.T) {
	full := &Agent{agentConfig: agentConfig{compactRatio: 0.8}, contextWindow: 100_000}
	if got := full.compactTrigger(); got != 80_000 {
		t.Fatalf("control trigger = %d, want 80000", got)
	}

	off := &Agent{agentConfig: agentConfig{compactRatio: 0.8}, contextWindow: 100_000, ablation: ablation.New(ablation.Compaction)}
